...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		Если Объект.Ссылка = УправлениеITОтделом8УФПовтИсп.ПолучитьЗначениеНастройки("СостояниеНовогоЗаказаПоставщику") Тогда
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Оповестить("Запись_СостоянияЗаказовПоставщикам");
...
Процедура УстановитьОсновнойЭлемент()
	...
	Если Объект.Ссылка <> УправлениеITОтделом8УФПовтИсп.ПолучитьЗначениеНастройки("СостояниеНовогоЗаказаПоставщику") Тогда
		УправлениеITОтделом8УФ.УстановитьНастройкуПользователя(Объект.Ссылка, "СостояниеНовогоЗаказаПоставщику");	
		Элементы.ФормаКомандаУстановитьОсновнойЭлемент.Заголовок = НСтр("ru = 'Используется при создании новых заказов'");
		Элементы.ФормаКомандаУстановитьОсновнойЭлемент.Доступность = Ложь;
	КонецЕсли; 
