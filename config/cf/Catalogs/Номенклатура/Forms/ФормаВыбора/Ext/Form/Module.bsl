...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Если хотим просмотра сразу всего доним списком
	Если Параметры.Свойство("ПросмотрСписком") И Параметры.ПросмотрСписком Тогда		
		ЕстьПараметрыПриОткрытии = Истина;
	КонецЕсли;
	...
	Если Параметры.Свойство("ОтборУслуги") Тогда		
		СписокНоменклатура.Параметры.УстановитьЗначениеПараметра("ОтборПоУслугам",Истина);
		СписокНоменклатура.Параметры.УстановитьЗначениеПараметра("ОтборУслуги",Параметры.ОтборУслуги);
	КонецЕсли;
	
	УстановитьЗначенияПоНастройкамФормы();
	
	Если ЕстьПараметрыПриОткрытии Тогда
		ПрименитьПараметрыПриОткрытии();
	КонецЕсли;
...
Процедура ИерархияНоменклатураПриАктивизацииСтроки(Элемент)
	Если Не ВариантНавигации = ПредопределенноеЗначение("Перечисление.ВариантыНавигацииВФормахНоменклатуры.ПоИерархии")	Тогда 
		Возврат;
	КонецЕсли;
...
Процедура ХарактеристикиЗначениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(ТД.ВидХарактеристики) Тогда
		Элементы.ХарактеристикиЗначение.ОграничениеТипа = Новый ОписаниеТипов(ПолучитьТипЗначенияХарактеристики(ТД.ВидХарактеристики));
	КонецЕсли;
...
Процедура УстановитьЗначенияПоНастройкамФормы()
	...
	Если Настройки <> Неопределено Тогда		
		...
		Если Настройки.Свойство("ТекущаяИерархияНоменклатуры")
			И ЗначениеЗаполнено(Настройки.ТекущаяИерархияНоменклатуры) Тогда
...
&НаСервере
Функция ПрименитьПараметрыПриОткрытии()		
	...
	Если Параметры.Свойство("ВыбранноеЗначение") И ЗначениеЗаполнено(Параметры.ВыбранноеЗначение) Тогда 
		Если ПоказыватьСкрыватьФильтры Тогда
			Если ВариантНавигации 			= Перечисления.ВариантыНавигацииВФормахНоменклатуры.ПоИерархии Тогда
				УдалитьОтборПоИерархииНоменклатуры();				
				УдалитьОтборПоВидуНоменклатурыИХарактеристикамВидаНоменклатуры();
				ТекущаяИерархияНоменклатуры = Параметры.ВыбранноеЗначение.Родитель;
				ИерархияНоменклатурыДоИзменения = Справочники.Номенклатура.ПустаяСсылка();
				ОтборПоИерархииПриИзмененииНаСервере();
				Элементы.СписокНоменклатура.ТекущаяСтрока = Параметры.ВыбранноеЗначение;			
			Иначе 
				УдалитьОтборПоИерархииНоменклатуры();
				УдалитьОтборПоВидуНоменклатурыИХарактеристикамВидаНоменклатуры();
				ВидНоменклатуры 			= Параметры.ВыбранноеЗначение.ВидНоменклатуры;
				ВидНоменклатурыДоИзменения 	= Справочники.ВидыНоменклатуры.ПустаяСсылка();
				ВидНоменклатурыПриИзмененииНаСервере();
				Элементы.СписокНоменклатура.ТекущаяСтрока = Параметры.ВыбранноеЗначение;
			КонецЕсли;
		Иначе
	...
	Если Параметры.Свойство("ПросмотрСписком") И ЗначениеЗаполнено(Параметры.ПросмотрСписком) Тогда 
	...
	Если Параметры.Свойство("СписокДляОтбораПоВидуНоменклатуры")
		И ЗначениеЗаполнено(Параметры.СписокДляОтбораПоВидуНоменклатуры) Тогда		
		...
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(
			ЭлементыОтбора,
			"ВидНоменклатуры",
			Параметры.СписокДляОтбораПоВидуНоменклатуры,
...
Процедура УстановитьВидимостьИДоступность()
	...
	Если ПросмотрСписком Тогда
		Элементы.ГруппаВариантыНавигации.Видимость	= Ложь;
		ПоказыватьСкрыватьФильтры					= Ложь;
		Элементы.ПоказатьСкрытьФильтры.Заголовок 	= НСтр("ru = '<<Показать фильтры'");
	Иначе	
...
Процедура УстановитьОтображениеСпискаНоменклатуры()
	...
	ИначеЕсли ИспользоватьФильтры Тогда
		Элементы.СписокНоменклатура.Отображение = ОтображениеТаблицы.Список;
	Иначе
...
&НаКлиенте
Процедура УстановитьТекущуюНоменклатуру()
	
	Если ЗначениеЗаполнено(НоменклатураПриОткрытии) Тогда
		Элементы.СписокНоменклатура.ТекущаяСтрока = НоменклатураПриОткрытии;	
	КонецЕсли;
