...
//<< УИТ Администратор - 24.01.2020 13:28:49
// Общие замечания. Для хранения используются отдельные реквизиты для Telegram (ТекстШаблонаTelegram, ПредназначенДляTelegram)
// А для отображения используются элементы формы для SMS сообщений.
//>>
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	//<< УИТ Добавили шаблоны Telegram
	ИначеЕсли ТекущийОбъект.ПредназначенДляTelegram Тогда
		ТекущийОбъект.ТекстШаблонаTelegram = ТелоСообщенияОбычныйТекст.ПолучитьТекст();
		ТекущийОбъект.ФорматВложений = Неопределено;
	//>>
	Иначе
...
Процедура ТелоСообщенияОбычныйТекстSMSПриИзменении(Элемент)
	//<< УИТ Администратор - 24.01.2020 15:04:12
	...
	Если Объект.ПредназначенДляTelegram Тогда
		Объект.ТекстШаблонаTelegram = ТелоСообщенияОбычныйТекст.ПолучитьТекст();
	Иначе
		Объект.ТекстШаблонаSMS = ТелоСообщенияОбычныйТекст.ПолучитьТекст();
		ТелоСообщенияОбычныйТекст.УстановитьТекст(Объект.ТекстШаблонаSMS); // Тех. ограничение SMS 1024 символа.
	КонецЕсли;	
...
Процедура УстановитьВыборФормата(Знач ФорматыСохранения = Неопределено)
	...
	Если Объект.ПредназначенДляSMS Тогда
		Возврат;
	КонецЕсли;
	//<< УИТ Администратор - 24.01.2020 13:19:57
	Если Объект.ПредназначенДляTelegram Тогда
		Возврат;
	КонецЕсли;
	//>>
...
Процедура ИнициализироватьНовыйШаблонСообщений(Знач НастройкиШаблоновСообщений)
	...
	ИначеЕсли Параметры.ПараметрыВыбора.Количество() > 0 Тогда
		...
		//<< УИТ Администратор - 24.01.2020 13:19:29		
		ИначеЕсли Параметры.ПараметрыВыбора.Свойство("ПредназначенДляTelegram")
			И Параметры.ПараметрыВыбора.ПредназначенДляTelegram Тогда
			ВидСообщения = "Telegram"
		КонецЕсли;		
		//>>
	...
	Если Параметры.Основание = Неопределено Тогда
		...
		//<< УИТ Администратор - 24.01.2020 13:22:23
		Объект.ПредназначенДляTelegram = Ложь;
		//>>
		
		Если ВидСообщения = "СообщениеSMS" Тогда
			Объект.ПредназначенДляSMS = Истина;
			Объект.ПредназначенДляЭлектронныхПисем = Ложь;
		//<< УИТ Администратор - 24.01.2020 13:21:52
		ИначеЕсли ВидСообщения = "Telegram" Тогда
			Объект.ПредназначенДляTelegram = Истина;
...
Процедура УстановитьТекстШаблона(ТекущийОбъект, СписокФайлов = Неопределено)
	...
	//<< УИТ Администратор - 24.01.2020 13:23:52
	ИначеЕсли ТекущийОбъект.ПредназначенДляTelegram Тогда
		ТелоСообщенияОбычныйТекст.УстановитьТекст(ТекущийОбъект.ТекстШаблонаTelegram);	
	//>>
	Иначе
		Если ТекущийОбъект.ТипТекстаПисьма = Перечисления.СпособыРедактированияЭлектронныхПисем.HTML Тогда
...
Процедура ОтображениеЭлементовФормы(ФорматаПисьма = "")
	...
	Если Объект.ПредназначенДляSMS Тогда
		...
		//<< УИТ Администратор - 24.01.2020 14:46:07
		Элементы.ДекорацияСообщениеSMS.Заголовок = НСтр("ru = 'Сообщение SMS'");
		Элементы.ОтправлятьВТранслите.Видимость = Истина;
		//>>
	...
	//<< УИТ Администратор - 24.01.2020 13:24:29
	ИначеЕсли Объект.ПредназначенДляTelegram Тогда
		СуффиксЗаголовка = НСтр("ru = 'Шаблон сообщения Telegram'");
		Элементы.ФормаВидТекстаЭлектронногоПисьма.Видимость = Ложь;
		Элементы.Страницы.ТекущаяСтраница = Элементы.СообщениеSMS;
		Элементы.ДекорацияСообщениеSMS.Заголовок = НСтр("ru = 'Сообщение Telegram (можно использовать теги <a>, <b>, <i>, <code>, <pre> и именованные HTML-сущности &lt; &gt; &amp; &quot;)'");
		Элементы.ДекорацияСообщениеSMS.АвтоМаксимальнаяШирина = Ложь;
...
Процедура ЗаполнитьШаблонПоВнешнейОбработке()
		...
		//<< УИТ Администратор - 24.01.2020 13:25:16
		Объект.ПредназначенДляTelegram              = Ложь;
		//>>
		...
		//<< УИТ Администратор - 24.01.2020 13:25:30
		ИначеЕсли СтруктураДанныхВнешнейОбработки.ПредназначенДляTelegram Тогда
			
			ТелоСообщенияОбычныйТекст.УстановитьТекст(СтруктураДанныхВнешнейОбработки.ТекстШаблонаTelegram);
			Объект.ПредназначенДляTelegram              = Истина;
			Объект.ПредназначенДляSMS              = Ложь;
			Объект.ПредназначенДляЭлектронныхПисем = Ложь;		
		//>>
...
Процедура ОчиститьШаблон(Форма)
	...
	//<< УИТ Администратор - 24.01.2020 13:26:30
	Форма.Объект.ПредназначенДляTelegram                 = Ложь;
	//>>
...
Процедура ДобавитьВТекстСообщенияПараметр()
	...
	Если Элементы.Реквизиты.ВыделенныеСтроки <> Неопределено Тогда
		...
		Иначе
			...
			//<< УИТ Администратор - 24.01.2020 13:27:04
			ИначеЕсли Объект.ПредназначенДляTelegram Тогда
				Элементы.ТелоСообщенияОбычныйТекстSMS.ВыделенныйТекст = Текст;
			//>>
			Иначе
