...
// Заголовок - Строка
// ТекстHTML - Строка
// ОбъектБазыЗнаний - СправочникСсылка.КатегорииБазыЗнаний, СтатьиБазыЗнаний, Комментарии
...
Функция ДобавитьНовость(знач Автор, знач Заголовок, знач ТекстHTML, знач ОбъектБазыЗнаний = Неопределено, знач ВидНовости = Неопределено, Получатели = Неопределено, знач Важность = 2, знач Текст = "") Экспорт 
	...
	Если ВидНовости = Перечисления.ВидыНовостейБазыЗнаний.Удаление Тогда
		ЭлементОбъект.ОбъектБазыЗнаний	= Справочники.Комментарии.ПустаяСсылка();
	Иначе	
...
Процедура ДополнитьПолучателейНовостиПоВидуИОбъекту(Получатели, ВидНовости, ОбъектБазыЗнаний)
	...
	ИначеЕсли ТипЗнч(ОбъектБазыЗнаний) = Тип("СправочникСсылка.Комментарии") Тогда
		СтатьяВладелец	= ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОбъектБазыЗнаний, "ВладелецКомментария");
