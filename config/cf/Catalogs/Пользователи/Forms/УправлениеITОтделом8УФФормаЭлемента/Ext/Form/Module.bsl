...
Процедура ПослеЗаписиЗавершение(Результат, ПараметрыЗаписи) Экспорт
	...
	// << УИТ. Функции УправлениеITОтделом8УФПовтИсп.СписокПодчиненныхСотрудников и
	// УправлениеITОтделом8УФПовтИсп.СписокПодчиненныхПользователей кэшируются. При изменении
	// Пользователей эти данные надо сбрасывать.
...
//<< УИТ
#Область УправлениеITОтделом

&НаКлиенте
Процедура СотрудникПриИзменении(Элемент)
	
	ОбновитьСотрудникаСервер();
	
КонецПроцедуры

&НаСервере
Процедура ОбновитьСотрудникаСервер()
	
	Если ЗначениеЗаполнено(Объект.Сотрудник) Тогда
		Объект.Наименование 	= Строка(Объект.Сотрудник);
		Объект.ФизическоеЛицо 	= Объект.Сотрудник.Физлицо;
...
Процедура ДобавитьГруппуПользователей(Команда)
	...
	ОткрытьФорму("Справочник.ГруппыПользователей.ФормаВыбора", ПараметрыФормы, ЭтаФорма,,,, Новый ОписаниеОповещения("ДобавитьГруппуПользователейЗавершение", ЭтотОбъект), РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
...
Процедура ЗаполнитьПользователейПоГруппе(ГруппаПользователей)
	...
	Для каждого Строки Из ГруппаПользователей.Состав Цикл
		НоваяСтрока 			= Объект.Подчиненные.Добавить();
		НоваяСтрока.Подчиненный = Строки.Пользователь;
	КонецЦикла;
