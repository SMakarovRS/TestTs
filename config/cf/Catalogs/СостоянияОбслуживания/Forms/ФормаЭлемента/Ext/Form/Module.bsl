...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		Если Объект.Ссылка = УправлениеITОтделом8УФПовтИсп.ПолучитьЗначениеНастройки("СостояниеНовогоОбслуживания") Тогда
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Оповестить("Запись_СостоянияНовогоОбслуживания");
...
Процедура КомандаУстановитьОсновнойЭлемент(Команда)
	...
	Иначе
		ПоказатьПредупреждение(,НСтр("ru='Элемент необходимо в начале записать.'"));
	КонецЕсли;
...
Процедура УстановитьОсновнойЭлемент()
	...
	Если Объект.Ссылка <> УправлениеITОтделом8УФПовтИсп.ПолучитьЗначениеНастройки("СостояниеНовогоОбслуживания") Тогда
		УправлениеITОтделом8УФ.УстановитьНастройкуПользователя(Объект.Ссылка, "СостояниеНовогоОбслуживания");	
		Элементы.ФормаКомандаУстановитьОсновнойЭлемент.Заголовок = НСтр("ru = 'Используется при создании новых ремонтов'");
		Элементы.ФормаКомандаУстановитьОсновнойЭлемент.Доступность = Ложь;
	КонецЕсли; 
