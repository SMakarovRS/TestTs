...
Процедура ЗаписатьИЗакрыть(Команда)
	...
	Результат = ПроверитьЗаполнениеФормы();
	...
	ИначеЕсли ЭтотОбъект.Тип = 5 Тогда
		ЭтотОбъект.Адрес = ЭтотОбъект.СтрокаПоиска;
	ИначеЕсли ЭтотОбъект.Тип = 6 Тогда
		ЭтотОбъект.Адрес = "tel:" + ЭтотОбъект.Телефон;
	КонецЕсли;
...
Процедура НастроитьФорму()
	...
	Элементы.Телефон.Видимость				= (ЭтотОбъект.Тип = 6);
	
	Если ЭтотОбъект.Тип = 2 Тогда
		Элементы.АдресСсылки.Заголовок 		= "E-mail";
		Элементы.АдресСсылки.ПодсказкаВвода = "support@mail.ru";
	Иначе 
		Элементы.АдресСсылки.Заголовок 		= "URL";
		Элементы.АдресСсылки.ПодсказкаВвода = "https://test.ru";
	КонецЕсли;
...
Процедура ИнициализацияДанныхСсылкиНаСервере()
	...
	ИначеЕсли ЭтотОбъект.Тип = 5 Тогда
		ЭтотОбъект.СтрокаПоиска = ЭтотОбъект.Адрес;
	ИначеЕсли ЭтотОбъект.Тип = 6 Тогда // Телефон
		ЭтотОбъект.Телефон = СокрЛП(ЭтотОбъект.Адрес);
	КонецЕсли;
...
Функция ПроверитьЗаполнениеФормы()
	...
	Если Тип = 0 Тогда
		МассивРеквизитов.Добавить("СтатьяБазыЗнаний");
		МассивРеквизитов.Добавить("Представление");			
	ИначеЕсли Тип = 1 Тогда
		МассивРеквизитов.Добавить("АдресСсылки");
		МассивРеквизитов.Добавить("Представление");
	ИначеЕсли Тип = 2 Тогда
		МассивРеквизитов.Добавить("АдресСсылки");
		МассивРеквизитов.Добавить("Представление");
	ИначеЕсли Тип = 3 Тогда
		МассивРеквизитов.Добавить("КатегорияБазыЗнаний");
		МассивРеквизитов.Добавить("Представление");
	ИначеЕсли Тип = 4 Тогда
		МассивРеквизитов.Добавить("СтрокаПоиска");
		МассивРеквизитов.Добавить("Представление");
	...
	Для Каждого ЭлементМассива Из МассивРеквизитов Цикл
		Если Не ЗначениеЗаполнено(ЭтотОбъект[ЭлементМассива]) Тогда
			Если Тип = 2 Тогда				
				ТекстОшибки = НСтр("ru = 'Поле ""E-mail"" не заполнено'");				
			Иначе	
				ТекстОшибки = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Поле", "Заполнение", ЭлементМассива);
			КонецЕсли;	
			ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстОшибки, , ЭлементМассива, , Результат);
