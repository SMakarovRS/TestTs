...
Процедура ЗаполнитьСписокВыбораДляДатыНачала(Элемент)
	...
	НачалоРабочегоДняДляСпискаВыбора = НачалоДня(пДата)+Час(Объект.НачалоРабочегоДня) * 60 * 60
		+ Минута(Объект.НачалоРабочегоДня) * 60;
	ОкончаниеРабочегоДня = НачалоДня(пДата) + (Час(Объект.НачалоРабочегоДня) 
		+ Объект.КоличествоРабочихЧасовВДне) * 60 * 60 + Минута(Объект.НачалоРабочегоДня) * 60;
	а = 0;
	Пока Истина Цикл
...
Процедура ЗаполнитьСписокВыбораДляДатыОкончания(Элемент)
	...
	Если Не ЗначениеЗаполнено(пДата) Тогда
		пДата = ТекущаяДата();
	КонецЕсли;
	НачалоРабочегоДняДляСпискаВыбора = НачалоДня(пДата)+(Час(Объект.НачалоРабочегоДня) + 1) * 60 * 60
		+ Минута(Объект.НачалоРабочегоДня) * 60;
	ОкончаниеРабочегоДня = НачалоДня(пДата) + (Час(Объект.НачалоРабочегоДня) 
		+ Объект.КоличествоРабочихЧасовВДне) * 60 * 60 + Минута(Объект.НачалоРабочегоДня) * 60;
	а = 0;
...
&НаКлиенте
Процедура СоздатьЗаданиеКИсполнению(Команда)
	
	ПараметрыЗадания = Новый Структура;
	ПараметрыЗадания.Вставить("ОткрываетсяИзВне", Истина);
	ОбъектЗадание = Новый Структура;
	ОбъектЗадание.Вставить("Тема", 		НазваниеЗадачи);
	ОбъектЗадание.Вставить("Проект", 	Объект.Проект);
	РеквизитыПроекта = РеквизитыПроекта();	
	ОбъектЗадание.Вставить("Инициатор", РеквизитыПроекта.Инициатор);
	ОбъектЗадание.Вставить("Клиент", 	РеквизитыПроекта.Клиент);
	ПараметрыЗадания.Вставить("Объект", ОбъектЗадание);
	ОткрытьФорму("Документ.Задание.ФормаОбъекта", ПараметрыЗадания, ЭтаФорма);
...
Функция СкорректироватьДатуОкончания(Знач пДата, Знач СдвигатьНазад = Истина)
	...
	ИначеЕсли Час(пДата) <= Час(Объект.НачалоРабочегоДня) и СдвигатьНазад = Ложь Тогда
		Возврат ВернутьДатуСУчетомВыходных(НачалоДня(пДата) 
			+ (Час(Объект.НачалоРабочегоДня) + Объект.КоличествоРабочихЧасовВДне) * 3600
			+ Минута(Объект.НачалоРабочегоДня) * 60, Истина);
	ИначеЕсли Час(пДата) > Час(Объект.НачалоРабочегоДня) + Объект.КоличествоРабочихЧасовВДне 
...
Функция РасчитатьРазницуМеждуДатами(Знач пНачало, Знач пОкончание) Экспорт 
	...
	Пока Начало<Окончание Цикл
		...
		ИначеЕсли Окончание - Начало >=(3600 * Объект.КоличествоРабочихЧасовВДне) Тогда
			...
			КонецЕсли;
		Иначе
			// Дробная часть будет в часах //Объект.КоличествоРабочихЧасовВДне);
...
Функция РасчитатьОкончание(Знач Начало, Знач перемДлительность) Экспорт
	...
	Если перемДлительность >= 0 Тогда
		Пока Счетчик > 0 Цикл
			...
			Иначе
				//Дробная часть будут часы //(3600 * Объект.КоличествоРабочихЧасовВДне)*Счетчик;
...
Функция РеквизитыПроекта()
	...
	Возврат ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Объект.Проект, "Инициатор, Клиент");
