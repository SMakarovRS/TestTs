...
Процедура ФоновоеЗаданиеЗапуститьНаКлиенте(Действие, ИмяЗадания, Отказ)
	...
	//<< УИТ Принудительная отмена выгрузки из конфигурации
	Если ТипЗнч(УзелИнформационнойБазы) = Тип("ПланОбменаСсылка.СинхронизацияДанныхЧерезУниверсальныйФормат") Тогда 
		Если ПараметрыЗадания.ВыполнятьВыгрузку = Истина Тогда 
			ОтменитьРегистрацию(УзелИнформационнойБазы);
		КонецЕсли;
	КонецЕсли;
	//>>
...
//<< УИТ
&НаСервере
Процедура ОтменитьРегистрацию(Знач УзелОбмена)
	...
	Если ВыборкаТипов.Количество() > 0 Тогда
		...
		Пока ВыборкаТипов.Следующий() Цикл
			...
			ЭлементыВыборка = ВыборкаТипов.Выбрать();
			
			Пока ЭлементыВыборка.Следующий() Цикл
				ПланыОбмена.УдалитьРегистрациюИзменений(УзелОбмена, ЭлементыВыборка.Ссылка);
			КонецЦикла;
