...
Процедура ВыполнитьНачальнуюВыгрузкуДанных(Команда)
	...
	//<< УИТ Добавлена проверка для запрета интерактивной выгрузки в формате обмена Enterprise Data
...
Процедура ВыполнитьСопоставлениеИЗагрузкуДанныхПродолжение(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	...
	Если ТекущийЭтапНастройки = "СопоставлениеИЗагрузка"
		...
		//<< УИТ Добавлена проверка для того чтобы пользователь не смог перейти на этап выгрузки
		Если ТипЗнч(УзелОбмена) <> Тип("ПланОбменаСсылка.СинхронизацияДанныхЧерезУниверсальныйФормат") Тогда 
			ПерейтиКСледующемуЭтапуНастройки();
		КонецЕсли;
		//>>
	КонецЕсли;
