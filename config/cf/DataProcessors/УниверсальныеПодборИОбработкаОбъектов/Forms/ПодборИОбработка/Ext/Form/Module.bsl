...
Процедура НайтиСсылки(Команда)
	Состояние(НСтр("ru = 'Поиск ссылок...'"));
	НайтиСсылкиПоОтбору();
	...
	Если РежимПоиска = 1 Тогда
		...
		Для Каждого СтрокаПараметров Из ПараметрыЗапроса Цикл
			Если СтрокаПараметров.ЭтоВыражение Тогда
				Запрос.УстановитьПараметр(СтрокаПараметров.ИмяПараметра, ОбщегоНазначения.ВычислитьВБезопасномРежиме(СтрокаПараметров.ЗначениеПараметра));
			Иначе
