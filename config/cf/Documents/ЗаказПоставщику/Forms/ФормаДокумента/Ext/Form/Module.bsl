...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("Документ") Тогда
		МассивСтрок = Объект.Основания.НайтиСтроки(Новый Структура("Основание", Параметры.Документ));
		Если МассивСтрок.Количество() = 0 Тогда
			НоваяСтрока = Объект.Основания.Добавить();
			НоваяСтрока.Основание = Параметры.Документ;
	...
	Элементы.НоменклатураЗагрузитьДанныеИзТСД.Видимость = ИспользоватьПодключаемоеОборудование;
...
Функция СуммаПоследнейПокупки(ДатаАктуальности, Организация, Контрагент, Номенклатура)
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ ПЕРВЫЕ 1
		|	ЕСТЬNULL(ЗаказПоставщикуНоменклатура.Цена, 0) КАК Цена
		|ИЗ
		|	Документ.ЗаказПоставщику.Номенклатура КАК ЗаказПоставщикуНоменклатура
		|ГДЕ
		|	ЗаказПоставщикуНоменклатура.Номенклатура = &Номенклатура
		|	И ЗаказПоставщикуНоменклатура.Ссылка.Дата < &Дата
		|	И ЗаказПоставщикуНоменклатура.Ссылка.Проведен = ИСТИНА
		|	И ЗаказПоставщикуНоменклатура.Ссылка.Организация = &Организация
		|	И ЗаказПоставщикуНоменклатура.Ссылка.Контрагент = &Контрагент
		|
		|УПОРЯДОЧИТЬ ПО
		|	ЗаказПоставщикуНоменклатура.Ссылка.Дата УБЫВ";
...
Процедура РазвернутьСвернутьТЧНаСервере()
	...
	Элементы.ГруппаСостояние.Видимость         	= ВидимостьЭлементов;
	Элементы.Страницы.ОтображениеСтраниц		= ?(ВидимостьЭлементов, ОтображениеСтраницФормы.ЗакладкиСверху, ОтображениеСтраницФормы.Нет);
...
Процедура ПодсказкаСостояниеНажатие(Элемент)
	...
	ПараметрыОткрытия.Вставить("КлючПодсказки", "ЗаказПоставщику_СостояниеЗаказа");
...
Процедура ЗагрузитьИзФайла(Команда)
	...
	ПараметрыЗагрузки.ПолноеИмяТабличнойЧасти = "ЗаказПоставщику.Номенклатура";
...
Процедура ЗагрузитьТоварыИзФайлаНаСервере(АдресЗагруженныхДанных)
	...
	Для каждого СтрокаТаблицы Из ЗагруженныеДанные Цикл 
		...
		Если НЕ ЗначениеЗаполнено(НоваяСтрокаТовары.СтавкаНДС) Тогда
			НоваяСтрокаТовары.СтавкаНДС	= УправлениеITОтделом8УФПовтИсп.ПолучитьЗначениеПоУмолчаниюПользователя(Пользователи.ТекущийПользователь(), "ОсновнаяСтавкаНДС");
		КонецЕсли;
		Если НЕ ЗначениеЗаполнено(НоваяСтрокаТовары.СтавкаНДС) Тогда
...
Процедура УстановитьВидимостьДоступность()
	...
	Элементы.ДатаПоступления.Видимость             = Объект.ПоложениеДатыПоступления = Перечисления.ПоложениеРеквизитаНаФорме.ВШапке;
	Элементы.НоменклатураДатаПоступления.Видимость = Объект.ПоложениеДатыПоступления = Перечисления.ПоложениеРеквизитаНаФорме.ВТабличнойЧасти;
	...
	Элементы.НоменклатураСтатьяДоходовРасходов.Видимость    = Объект.ПоложениеСтатьиДоходовРасходов = Перечисления.ПоложениеРеквизитаНаФорме.ВТабличнойЧасти;
	Элементы.НоменклатураПодразделение.Видимость   = Объект.ПоложениеПодразделения = Перечисления.ПоложениеРеквизитаНаФорме.ВТабличнойЧасти;
	Элементы.НадписьТендер.Видимость			   = Объект.ВидОперации = Перечисления.ВидыОперацийЗаказПоставщику.Тендер;
...
Процедура ПересчитатьКурсКратностьВалютыРасчетов(Знач Оповещение, СтруктураДанные)
	...
	Если Объект.Курс <> КурсНовый
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'На дату документа у валюты расчетов (%1, ) был задан курс.
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ПересчитатьКурсКратностьВалютыРасчетовЗавершение", ЭтотОбъект, Новый Структура("КратностьНовый, КурсНовый, Оповещение", КратностьНовый, КурсНовый, Оповещение)), ТекстСообщения, Режим, 0);
        Возврат;		
...
Процедура ОбработатьИзмененияПоКнопкеЦеныИВалютыЗавершение(Результат, ДополнительныеПараметры) Экспорт
    ...
    Если ТипЗнч(СтруктураЦеныИВалюта) = Тип("Структура") И СтруктураЦеныИВалюта.БылиВнесеныИзменения Тогда
        ...
        Если НЕ СтруктураЦеныИВалюта.ПерезаполнитьЦены
            ...
            УправлениеITОтделом8УФКлиент.ПересчитатьЦеныТабличнойЧастиПоВалюте(ЭтаФорма, ВалютаРасчетовПередИзменением, "Номенклатура");
        КонецЕсли;
...
Функция ПолучитьДанныеНоменклатураПриИзменении(СтруктураДанные)
	...
	Если ЗначениеЗаполнено(СтруктураДанные.Номенклатура) Тогда
		...
		КонецЕсли;
		Цена = СуммаПоследнейПокупки(ДатаАктуальности, СтруктураДанные.Организация, СтруктураДанные.Контрагент, СтруктураДанные.Номенклатура);
	КонецЕсли;
...
Функция ЗаполнитьПоДаннымШтрихкодов(ДанныеШтрикодов)
	...
	Для каждого ТекШтрихкод Из СтруктураДанные.МассивШтрихкодов Цикл
		...
		Иначе
			...
			МассивСтрокТЧ = Объект.Номенклатура.НайтиСтроки(Новый Структура("Номенклатура,ЕдиницаИзмерения", НоменклатураШтрихкода, ЕдиницаИзмеренияШтрихкода));
			Если МассивСтрокТЧ.Количество() = 0 Тогда
				...
				НоваяСтрока.ЕдиницаИзмерения	= ?(ЗначениеЗаполнено(ЕдиницаИзмеренияШтрихкода), ЕдиницаИзмеренияШтрихкода, ДанныеШтрихкода.СтруктураДанныеНоменклатуры.ЕдиницаИзмерения);
				...
				Элементы.Номенклатура.ТекущаяСтрока	= НоваяСтрока.ПолучитьИдентификатор();
			Иначе
				...
				Элементы.Номенклатура.ТекущаяСтрока = НайденнаяСтрока.ПолучитьИдентификатор();
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
&НаКлиенте
Процедура ПолученыШтрихкодыФрагмент(НеизвестныеШтрихкоды)
	
	Для каждого ТекНеизвестныйШтрихкод Из НеизвестныеШтрихкоды Цикл
				
		СтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Данные по штрихкоду не найдены: %1; количество: %2'"),
