...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Для Каждого Строки Из Объект.Сотрудники Цикл
		МассивМестХранения.Добавить(Строки.СтароеМестоХранения);
	КонецЦикла;
	Оповестить("Запись_ПеремещениеСотрудников",
	...
	Если ПараметрыЗаписи.РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
        ОценкаПроизводительностиКлиент.УстановитьКлючевуюОперациюЗамера(ИдентификаторЗамераПроведение, 
			"ДокументПеремещениеСотрудников (проведение)");	
	КонецЕсли;
	// Конец СтандартныеПодсистемы.ОценкаПроизводительности
...
Процедура СотрудникиМестоХраненияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Форма = ОткрытьФорму("Справочник.МестаХранения.ФормаВыбора", Структура, Элемент,  Новый УникальныйИдентификатор());
	Форма.Открыть();
	
КонецПроцедуры

&НаКлиенте
Процедура СотрудникиСтароеМестоХраненияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	                                                                                                  
	Массив = УправлениеITОтделом8УФ.ПолучитьМассивМестХраненияСотрудника(?(Объект.Ссылка.Пустая(), 
		КонецДня(УправлениеITОтделом8УФ.ТекущаяДатаСистемная()), Объект.Дата), 
		Элементы.Сотрудники.ТекущиеДанные.Сотрудник);
	Элементы.СотрудникиСтароеМестоХранения.СписокВыбора.Очистить();
	Элементы.СотрудникиСтароеМестоХранения.СписокВыбора.ЗагрузитьЗначения(Массив);
...
Процедура СотрудникиСотрудникПриИзменении(Элемент)
	...
	Если ТекДанные <> Неопределено Тогда
		Если НЕ ЗначениеЗаполнено(ТекДанные.Сотрудник) Тогда
			ТекДанные.СтароеМестоХранения = ПредопределенноеЗначение("Справочник.МестаХранения.ПустаяСсылка");
			ТекДанные.НовоеМестоХранения = ПредопределенноеЗначение("Справочник.МестаХранения.ПустаяСсылка");
		КонецЕсли;
	КонецЕсли;
