...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ОкончаниеОбслуживания_ФормаСписка_УстановленОтборПериод" Тогда	
...
Процедура ОтборВидОбслуживанияОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	СтандартнаяОбработка	= Ложь;
	УстановитьОтборСписка("ВидОкончанияОбслуживания", Элемент.Родитель.Имя, ВыбранноеЗначение);
	ВыбранноеЗначение		= Неопределено;
...
Процедура СписокВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	КлючеваяОперация = "ДокументОкончаниеОбслуживания (открытие)";
...
Процедура УстановитьСтатус(Команда)
	...
	СтруктураСтрок		= Новый Структура("ВыделенныеСтроки,Состояние",ВыделенныеСтроки,Состояние);	
	ОписаниеОповещения 	= Новый ОписаниеОповещения("УстановитьСтатусЗавершение",ЭтаФорма,СтруктураСтрок);
...
Процедура УстановитьСтатусЗавершение(РезультатВопроса,ДопПараметры) Экспорт
	...
	Если РезультатВопроса <> КодВозвратаДиалога.Да Тогда
		Возврат;
	КонецЕсли;	
	УстановитьСтатусыУДокументов(ДопПараметры.ВыделенныеСтроки, ДопПараметры.Состояние);	
...
Процедура ЗакрытьЗаказы(Команда)
	...
	Режим		 = РежимДиалогаВопрос.ДаНет;
	СтруктураСтрок		= Новый Структура("ВыделенныеСтроки",ВыделенныеСтроки);	
	ОписаниеОповещения 	= Новый ОписаниеОповещения("ЗакрытьЗаказыЗавершение",ЭтаФорма,СтруктураСтрок);
	ПоказатьВопрос(ОписаниеОповещения,ТекстВопроса,Режим);	
...
Процедура СнятьСтатусЗакрыт(Команда)
	...
	Режим		 = РежимДиалогаВопрос.ДаНет;
	СтруктураСтрок		= Новый Структура("ВыделенныеСтроки",ВыделенныеСтроки);	
	ОписаниеОповещения 	= Новый ОписаниеОповещения("СнятьСтатусЗакрытЗавершение",ЭтаФорма,СтруктураСтрок);
	ПоказатьВопрос(ОписаниеОповещения,ТекстВопроса,Режим);
...
Процедура ПредставлениеПериодаНажатие(Элемент, СтандартнаяОбработка)
	...
	ДопПараметры.Вставить("ОписаниеОповещенияОВыбореПериода", 
		"ОкончаниеОбслуживания_ФормаСписка_УстановленОтборПериод");	
