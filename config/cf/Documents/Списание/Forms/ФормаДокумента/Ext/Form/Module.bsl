...
Процедура ДеревоНоменклатурыПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	...
	Если Не Копирование И УправлениеITОтделом8УФПовтИсп.НоменклатураМожетИметьКомплектующие(СтрокаТабличнойЧасти.Номенклатура) Тогда
		...
		ОписаниеОповещения = Новый ОписаниеОповещения("ДеревоНоменклатурыПередНачаломДобавленияЗавершение", ЭтаФорма, Структура);
...
Процедура ПодборНаСервере(АдресТЗ)
	...
	Для каждого Строки Из ТЗ Цикл
		...
		Для Каждого СтрокиМассива Из Массив Цикл
			ЗаполнитьЗначенияСвойств(НоваяСтрока,  СтрокиМассива, "Описание,Подразделение,СтатьяДоходовРасходов,ПричинаСписания");
		КонецЦикла;
...
Процедура Подбор(Команда)
	...
	// СтандартныеПодсистемы.ОценкаПроизводительности
	ОценкаПроизводительностиКлиент.УстановитьКлючевуюОперациюЗамера(ИдентификаторЗамераПроведение, "ДокументСписание (подборОткрытие)");
	// Конец СтандартныеПодсистемы.ОценкаПроизводительности
...
Процедура ПодборПоШтрихКоду(Команда)
	...
	ТекШтрихкод = "";
	ПоказатьВводЗначения(Новый ОписаниеОповещения("ПодборПоШтрихКодуЗавершение", ЭтотОбъект, Новый Структура("ТекШтрихкод", ТекШтрихкод)), ТекШтрихкод, НСтр("ru = 'Введите штрихкод'"));

КонецПроцедуры

&НаКлиенте
Процедура ПодборПоШтрихКодуЗавершение(Значение, ДополнительныеПараметры) Экспорт
...
Процедура УстановитьВидимостьДоступность()
	...
	Элементы.ДеревоНоменклатурыПодразделение.Видимость		= НЕ Элементы.Подразделение.Видимость;
	Элементы.СтатьяДоходовРасходов.Видимость							= (Объект.ПоложениеСтатьиДоходовРасходов = Перечисления.ПоложениеРеквизитаНаФорме.ВШапке) ИЛИ НЕ ЗначениеЗаполнено(Объект.ПоложениеСтатьиДоходовРасходов);
	Элементы.ДеревоНоменклатурыСтатьяДоходовРасходов.Видимость		= НЕ Элементы.СтатьяДоходовРасходов.Видимость;
...
Функция ЗаполнитьПодчиненнуюНоменклатуру(Период, НоменклатураВладелец)	
	...
	Пока Выборка.Следующий() Цикл
		...
		ПодчиненнаяНоменклатура.Вставить("Количество",			Выборка.Количество);
		...
		Если МассивСтрокТЧ.Количество() = 0 Тогда
			...
			НоваяСтрока.Количество		= ПодчиненнаяНоменклатура.Количество;			
		Иначе
			НайденнаяСтрока				= МассивСтрокТЧ[0];
			НайденнаяСтрока.Количество	= НайденнаяСтрока.Количество + ПодчиненнаяНоменклатура.Количество;						
		КонецЕсли;
		...
		Если СтруктураНоменклатуры.Свойство("НуженВыводДерева") И СтруктураНоменклатуры.Свойство("СвернутыйКомплект") Тогда
			Если СтруктураНоменклатуры.НуженВыводДерева И СтруктураНоменклатуры.СвернутыйКомплект Тогда 					
				...
				Если ЗначениеЗаполнено(СтруктураНоменклатуры.КарточкаНоменклатуры) Тогда
					...
					ОбщегоНазначенияКлиент.СообщитьПользователю(СтрШаблон(НСтр("ru = 'Номенклатура ""%1""  является частью комплекта. Комплект был добавлен в документ.'"), СтруктураНоменклатуры.НоменклатураШК, СтруктураНоменклатуры.Номенклатура));
				Иначе
			...
			ИначеЕсли СтруктураНоменклатуры.НуженВыводДерева И (Не СтруктураНоменклатуры.СвернутыйКомплект) Тогда					
				...
				Если ЗначениеЗаполнено(СтруктураНоменклатуры.КарточкаНоменклатуры) Тогда
					...
					ИдентификаторСтроки,ДеревоНоменклатуры.ПолучитьЭлементы(),СтруктураНоменклатуры.КарточкаНоменклатуры,Ложь);		
					Элементы.ДеревоНоменклатуры.ТекущаяСтрока = ИдентификаторСтроки;
					ОбщегоНазначенияКлиент.СообщитьПользователю(СтрШаблон(НСтр("ru = 'Номенклатура ""%1""  является частью комплекта. Комплект и все его комплектующие были добавлены в документ.'"), СтруктураНоменклатуры.НоменклатураШК, СтруктураНоменклатуры.Номенклатура));
				Иначе
					ОбщегоНазначенияКлиентСервер.ПолучитьИдентификаторСтрокиДереваПоЗначениюПоля("Номенклатура",
					ИдентификаторСтроки,ДеревоНоменклатуры.ПолучитьЭлементы(),СтруктураНоменклатуры.Номенклатура,Ложь);		
					Элементы.ДеревоНоменклатуры.ТекущаяСтрока = ИдентификаторСтроки;
