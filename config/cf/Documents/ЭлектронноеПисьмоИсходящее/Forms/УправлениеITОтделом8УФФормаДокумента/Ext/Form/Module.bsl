...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//<< УИТ.
	БазаЗнанийHTMLВызовСервера.ЭлектронноеПисьмоИсходящееПриСозданииНаСервере(ЭтаФорма, Параметры);
	ЗаданияСервер.ЭлектронноеПисьмоИсходящееПриСозданииНаСервере(ЭтаФорма, Параметры);
	//>>
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, РежимЗаписи, РежимПроведения)
	...
	КонецЕсли;
	Если НЕ ПустаяСтрока(Объект.ЭкспортируемыйТекстHTML) Тогда
		ЗаданияСервер.ЭлектронноеПисьмоИсходящееПередЗаписьюНаСервере(ЭтаФорма, ТекущийОбъект.ТекстHTML);
	КонецЕсли;
	//>>
...
Процедура ОчиститьСтатьюБазыЗнаний(Команда)
	...
	Если Не ПустаяСтрока(СтатьяБазыЗнанийHTML) Тогда
		...
		ПоказатьВопрос(ОписаниеОповещения, НСтр("ru = 'Текущая статья базы знаний будет очищена. Продолжить?'"), РежимДиалогаВопрос.ДаНет);
		Возврат;
	Иначе
...
Процедура ОчиститьСтатьюБазыЗнанийФрагмент()
	...
	Объект.СтатьяБазыЗнаний = ПредопределенноеЗначение("Справочник.СтатьиБазыЗнаний.ПустаяСсылка");	
...
Процедура ВыбратьСтатьюБазыЗнаний(Команда)
	...
	Если Не ПустаяСтрока(СтатьяБазыЗнанийHTML) Тогда
		...
		ПоказатьВопрос(ОписаниеОповещения, НСтр("ru = 'Текущая статья базы знаний будет заменена. Продолжить?'"), РежимДиалогаВопрос.ДаНет);
		Возврат;
	Иначе
...
Процедура ВыбратьСтатьюБазыЗнанийФрагмент()
	...
	ОткрытьФорму("Справочник.СтатьиБазыЗнаний.Форма.ФормаСписка", ПараметрыФормы, ЭтотОбъект, , , , ОписаниеОповещения, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);	
...
Процедура ПослеВыбораСтатьиБазыЗнаний(Результат, ДополнительныеПараметры) Экспорт
	...
	ОчиститьСтатьюБазыЗнанийФрагмент();		
	Объект.СтатьяБазыЗнаний = Результат;
...
Процедура ЗаполнитьТекстHTMLСтатьиНаСервере()
	...
	Если Объект.СтатьяБазыЗнаний.РазделИмпортирован Тогда
		СтатьяБазыЗнанийHTML = БазаЗнанийHTMLВызовСервера.ПолучитьТекстСтатьиИмпортированнойИзФайла(Объект.СтатьяБазыЗнаний);
	Иначе
		СтатьяБазыЗнанийHTML = БазаЗнанийHTMLВызовСервера.ПолучитьТекстСтатьиБазыЗнаний(Объект.СтатьяБазыЗнаний);
	КонецЕсли;
	
	Если Объект.СтатьяБазыЗнаний.Файлы.Количество() > 0 Тогда		
		Для Каждого СтрокаФайл Из Объект.СтатьяБазыЗнаний.Файлы Цикл			
			Попытка
				ДанныеФайла                      = РаботаСФайлами.ДанныеФайла(СтрокаФайл.Файл, Новый Структура("ИдентификаторФормы", УникальныйИдентификатор)); 				
				НоваяСтрока                      = Вложения.Добавить();
				НоваяСтрока.Расположение         = 4;
