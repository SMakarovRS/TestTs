...
Процедура ПослеНачалаРаботыСистемы() Экспорт
	...
	//<< УИТ Диалог новости об обновлениях.
	ПараметрыРаботыКлиента = СтандартныеПодсистемыКлиентПовтИсп.ПараметрыРаботыКлиентаПриЗапуске();
	
	Если ПараметрыРаботыКлиента.ДоступноИспользованиеРазделенныхДанных Тогда
		
		Если ПараметрыРаботыКлиента.СтруктураОписанияОбновлений.НеобходимоОбновление Тогда
			
			Структура = Новый Структура(ПараметрыРаботыКлиента.СтруктураОписанияОбновлений);
			ОткрытьФорму("ОбщаяФорма.УправлениеITОтделом8УФМониторПоддержки",Структура,,,,,,РежимОткрытияОкнаФормы.Независимый);
	...
	//<< УИТ Контроль обновления сброса повторно используемых значений.
	КонтрольОбновленияПовторноИспользуемыхЗначений();
	//>>
...
Процедура ПриУстановкеЗаголовкаКлиентскогоПриложения(ЗаголовокПриложения, ПриЗапуске) Экспорт
	...
	//<< УИТ Установка заголовка
	ВладелецЛицензии 		= УправлениеITОтделом8УФ.ВладелецЛицензии();
	РедакцияКонфигурации 	= УправлениеITОтделом8УФ.РедакцияКонфигурации();
	Если НЕ ПустаяСтрока(ВладелецЛицензии) Тогда			
		ЗаголовокПриложения = ЗаголовокПриложения + " " + СтрШаблон("%1 (%2)", РедакцияКонфигурации, ВладелецЛицензии);
	КонецЕсли;
	//>>	
