...
Функция СформироватьСообщение(ПараметрыОтправки) Экспорт
	...
	Иначе
		...
		//<< УИТ Барилко Виталий - 28.01.2020 14:41:43
		ИначеЕсли ПараметрыОтправки.Шаблон.ПредназначенДляTelegram Тогда
			ПараметрыОтправки.ДополнительныеПараметры.Вставить("ВидСообщения", "Telegram");		
		//>>
		Иначе
...
Функция ВидыШаблонов() Экспорт
	...
	//<< УИТ Барилко Виталий - 29.01.2020 15:49:00
	ТипыШаблонов.Добавить("Telegram", НСтр("ru = 'Telegram'"));
	//>>	
...
Функция ПодготовитьЗапросДляПолученияСпискаШаблонов(ТипШаблона, Предмет = Неопределено, ВладелецШаблона = Неопределено, ВыводитьОбщиеШаблон = Истина) Экспорт
	...
	//<< УИТ Сайдаков Сергей - 22.04.2020 15:43:16
	|	И НЕ ШаблоныСообщений.Ссылка В
	|			(ВЫБРАТЬ
...
Функция ПараметрыШаблона(Шаблон) Экспорт
	...
	Если ТипЗнч(Шаблон) = Тип("ДанныеФормыСтруктура") Тогда
		...
		//<< УИТ Барилко Виталий - 28.01.2020 14:30:04
		Если Шаблон.ПредназначенДляTelegram Тогда
			Результат.ТипШаблона              = "Telegram";
		КонецЕсли;
		//>>
		...
		//<< УИТ Барилко Виталий - 28.01.2020 14:31:35
		ИначеЕсли Результат.ТипШаблона = "Telegram" Тогда
			Результат.Текст                    = Шаблон.ТекстШаблонаTelegram;		
		//>>
	...
	ИначеЕсли ТипЗнч(Шаблон) = Тип("СправочникСсылка.ШаблоныСообщений") Тогда
		...
		//<< УИТ Барилко Виталий - 24.01.2020 18:08:54
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ШаблоныСообщений.ПредназначенДляSMS,", 
			"ШаблоныСообщений.ПредназначенДляSMS, ШаблоныСообщений.ПредназначенДляTelegram, ШаблоныСообщений.ТекстШаблонаTelegram,");
		//>>
		...
		Если СведенияОШаблоне.Количество() > 0 Тогда
			...
			КонецЦикла;
			//<< УИТ Барилко Виталий - 24.01.2020 18:10:36
			// Результат.Текст                      = СтрокаСведенийОШаблоне.ТекстШаблона;
			Если СтрокаСведенийОШаблоне.ПредназначенДляTelegram Тогда
				Результат.Текст	= СтрокаСведенийОШаблоне.ТекстШаблонаTelegram;
			Иначе
				Результат.Текст	= СтрокаСведенийОШаблоне.ТекстШаблона;
			КонецЕсли;
			//>>
			...
			//<< УИТ Барилко Виталий - 28.01.2020 14:32:38
			Если СтрокаСведенийОШаблоне.ПредназначенДляTelegram Тогда
				Результат.ТипШаблона             = "Telegram";
			КонецЕсли;
			//>>
...
Функция ПараметрыИзТекстаСообщения(ПараметрыШаблона) Экспорт
	...
	//<< УИТ Барилко Виталий - 28.01.2020 14:28:56
	ИначеЕсли ПараметрыШаблона.ТипШаблона = "Telegram" Тогда
		Возврат ОпределитьПараметрыТекстаСообщения(ПараметрыШаблона.Текст);	
	//>>
	Иначе
...
Процедура ЗаполнитьРеквизитыПоПредмету(Реквизиты, Предмет)
	...
	Если ЗначениеЗаполнено(СписокРеквизитов) Тогда
		...
		//<< УИТ Сайдаков Сергей - 01.12.2020 14:22:00
		Для Каждого Строка Из Реквизиты Цикл
			Если Строка.Ключ = "Уважаемыйая" И Строка.Значение = "" Тогда
					СтрокаРезультата = НСтр("ru = 'Уважаемый(ая)'");
					Реквизиты.Вставить("Уважаемыйая", СтрокаРезультата);
			КонецЕсли;
			
			Если Строка.Ключ = "НомерЧислом" И Строка.Значение = "" Тогда 
				Реквизиты.Вставить("НомерЧислом", УправлениеITОтделом8УФ.ДокументНомерЧислом(Предмет.Номер));					
			КонецЕсли;
		КонецЦикла;
		//>>
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаСФайлами") Тогда
		...
		Для каждого Картинка Из ДокументHTML.Картинки Цикл
			...
			Если ИзображениеОтсутствуетВПрисоединенныхФайлах Тогда
				//<< УИТ Барилко Виталий (добавлена Попытка - Исключение).
				Попытка
...
Процедура ЗаполнитьПолучателейСообщения(ПараметрыОтправки, ПараметрыШаблона, Результат, МенеджерОбъекта)
	...
	//<< УИТ Барилко Виталий - 28.01.2020 15:01:43
	ИначеЕсли ПараметрыШаблона.ТипШаблона = "Telegram" Тогда
		...
		Если РасширенныйСписокПолучателей Или (ПараметрыШаблона.Свойство("РасширенныйСписокПолучателей")
			...
			Для каждого Получатель Из Получатели Цикл
				Если ЗначениеЗаполнено(Получатель.НомерТелефона) Тогда
					ЗначениеПолучателя = Новый Структура("ИмяПрофиля, Представление, ИсточникКонтактнойИнформации", 
					Получатель.ИмяПрофиля, Получатель.Представление, Получатель.Контакт);
					Результат.Получатель.Добавить(ЗначениеПолучателя);
				КонецЕсли;
			КонецЦикла;
		...
		Иначе
			...
			Результат.Получатель = Новый СписокЗначений;
			Для каждого Получатели Из Получатели Цикл
				Если ЗначениеЗаполнено(Получатели.ИмяПрофиля) Тогда
					Результат.Получатель.Добавить(Получатели.ИмяПрофиля, Получатели.Представление);
				КонецЕсли;
			КонецЦикла;
...
Функция СформироватьПолучателейПоУмолчанию(Предмет, ТипШаблона);
	...
	//<< УИТ Барилко Виталий - 28.01.2020 15:03:54
	ИначеЕсли СтрСравнить(ТипШаблона, "Telegram") = 0 Тогда
		Получатели.Колонки.Добавить("ИмяПрофиля", Новый ОписаниеТипов("Строка"));
		ИмяКолонки = "ИмяПрофиля";
	//>>
	Иначе
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.КонтактнаяИнформация") Тогда
		...
		ТипКонтактнойИнформации = ?(СтрСравнить(ТипШаблона, "SMS") = 0,
			...
			//<< УИТ Барилко Виталий - 28.01.2020 15:10:00
			Если СтрСравнить(ТипШаблона, "Telegram") = 0 Тогда
				ТипКонтактнойИнформации = МодульУправлениеКонтактнойИнформацией.ТипКонтактнойИнформацииПоНаименованию("Telegram");
			КонецЕсли;			
			//>>
