...
Функция ОбработатьОшибкуОтправкиПисьма(Знач ПисьмоОбъект, Знач ВыборкаПисьма, Знач ПараметрыПисьма, Знач Интерактивно)
	...
	//<< УИТ Сайдаков Сергей - 25.11.2020 17:38:15
	Если ПараметрыПисьма = Неопределено Тогда
		ПараметрыПисьма = Новый Структура();
	КонецЕсли; 
	
	Если НЕ ПараметрыПисьма.Свойство("ОшибочныеПолучатели") Тогда
		ПараметрыПисьма.Вставить("ОшибочныеПолучатели", Новый Соответствие);
	КонецЕсли;
	//>>
...
Процедура ПолучитьПисьмаПоМассивуИдентификаторов(Почта,
	...
	Если ИдЗагружать.Количество() <> 0 Тогда
		...
		Пока ИдЗагружать.Количество() > (Получено + КоличествоОшибокПриЗаписи + КоличествоНеАктуальныхСообщений) Цикл
			...
			//<< УИТ Не загружаем письма, которые меньше даты начальной загрузки.
			ДатаНачальнойЗагрузки = ДанныеУчетнойЗаписи.Ссылка.ДатаНачальнойЗагрузки;
			//>>
			...
			Для Каждого Сообщение Из Сообщения Цикл
				...
				//<< УИТ Не загружаем письма, которые меньше даты начальной загрузки.
				Если ДатаНачальнойЗагрузки <> Дата(1, 1, 1) Тогда					
					Попытка
						Если Сообщение.ДатаПолучения <> Дата(1, 1, 1)
							И Сообщение.ДатаПолучения < ДатаНачальнойЗагрузки Тогда
							Получено = Получено + 1;
							Продолжить;
						КонецЕсли;
					Исключение
					...
					Попытка
						Если Сообщение.ДатаОтправления <> Дата(1, 1, 1) 
							И Сообщение.ДатаОтправления < ДатаНачальнойЗагрузки Тогда
							Получено = Получено + 1;
							Продолжить;
						КонецЕсли;
					Исключение
...
Процедура ПолучитьПочтуПоПроколуIMAP(ДанныеУчетнойЗаписи, Почта, Получено, ПолученныеПисьма)
	...
	Для каждого ИмяАктивнойПапки Из МассивИменАктивныхПапок Цикл
		...
		//<< УИТ Если дата начальной загрузки задана в настройках учетной записи.
		// Так же возможен случай, когда Imap и дата в регистре больше даты в настройках.
		Если ДанныеУчетнойЗаписи.Ссылка.ДатаНачальнойЗагрузки <> Дата(1,1,1) Тогда
			Если ДанныеУчетнойЗаписи.ДатаЗагрузкиПисем <> Дата(1,1,1)
				И ДанныеУчетнойЗаписи.ДатаЗагрузкиПисем < ДанныеУчетнойЗаписи.Ссылка.ДатаНачальнойЗагрузки Тогда 
				ПараметрыОтбора.Вставить("ПослеДатыОтправления", ДанныеУчетнойЗаписи.Ссылка.ДатаНачальнойЗагрузки);
			Иначе
...
Функция ЗаписатьЭлектронноеПисьмо(ДанныеУчетнойЗаписи,
	...
	//<< УИТ Присваиваем новую ссылку, новому объекту.
	Если ЭтоИсходящееПисьмо Тогда
		НоваяСсылкаПисьмо = Документы.ЭлектронноеПисьмоИсходящее.ПолучитьСсылку(Новый УникальныйИдентификатор);
	Иначе
		НоваяСсылкаПисьмо = Документы.ЭлектронноеПисьмоВходящее.ПолучитьСсылку(Новый УникальныйИдентификатор);
	КонецЕсли;	
	Письмо.УстановитьСсылкуНового(НоваяСсылкаПисьмо);
	//>>
	...
	//<< УИТ Не присваиваем предмет.	
	//СтруктураДляЗаписи.Предмет                 = ?(ЗначениеЗаполнено(Предмет),Предмет,Письмо.Ссылка);
	СтруктураДляЗаписи.Предмет                 = ?(ЗначениеЗаполнено(Предмет),Предмет,Документы.ЭлектронноеПисьмоВходящее.ПустаяСсылка());
	//>>

	СтруктураДляЗаписи.Рассмотрено             = ПризнакРассмотрено;
	СтруктураДляЗаписи.РассчитыватьРассмотрено = Ложь;
	
	//<< УИТ предмет взаимодействий для новой ссылки
	РегистрыСведений.ПредметыПапкиВзаимодействий.ЗаписатьПредметыПапкиВзаимодействий(Письмо.ПолучитьСсылкуНового(), СтруктураДляЗаписи);
	...
	Если Не ЭтоИсходящееПисьмо И Письмо.УведомитьОПрочтении Тогда
		...
		//<< УИТ
		ЗаписатьНеобходимостьОбработкиУведомленияОПрочтении(Письмо.ПолучитьСсылкуНового());
...
Процедура ЗаполнитьДокументЭлектронноеПисьмо(Письмо, Сообщение, ЭтоИсходящееПисьмо)
	...
	//<< УИТ Исправляем дату получения с учетом часового пояса.
	//Письмо.Дата = ?(Сообщение.ДатаОтправления = Дата(1,1,1), ТекущаяДатаСеанса(), Сообщение.ДатаОтправления);
	Если Сообщение.ДатаОтправления = Дата(1,1,1) Тогда
		Письмо.Дата = ТекущаяДатаСеанса();
	Иначе
		ДатаОтправленияUTC = Сообщение.ДатаОтправления - Сообщение.СмещениеДатыОтправления;
		СмещениеПолучателя = СмещениеСтандартногоВремени(ЧасовойПоясСеанса(), ДатаОтправленияUTC) + СмещениеЛетнегоВремени(ЧасовойПоясСеанса(), ДатаОтправленияUTC);		
		Письмо.Дата = Сообщение.ДатаОтправления + (СмещениеПолучателя - Сообщение.СмещениеДатыОтправления);
	КонецЕсли;	
	//>>
...
Процедура ЗаполнитьПредметИКонтакты(Письмо,
	...
	//<< УИТ Если получаем письма, и предмет не задан, не заполняем его.
...
Процедура ЗаписатьВложениеЭлектронногоПисьма(Объект, Вложение,МассивПодписей,КоличествоПустыхИменВоВложениях)
	...
	//<< УИТ Для новых объектов исключение
	Если Объект.ЭтоНовый() Тогда
		ПисьмоСсылка = Объект.ПолучитьСсылкуНового();
	Иначе
