...
Процедура ЗаполнитьДополнительныеДанныеФайла(Результат, ПрисоединенныйФайл, ВерсияФайла = Неопределено) Экспорт
    ...
    //<< УИТ АдминистраторДом - 14.04.2020 13:05:50
    // КодировкаТекущейВерсии = ПолучитьКодировкуВерсииФайла(ТекущаяВерсияФайла);
    Если НЕ ПустаяСтрока(ТекущаяВерсияОбъект.Расширение) 
        И УправлениеITОтделом8УФ.ЭтоКартинка(ТекущаяВерсияОбъект.Расширение) Тогда
		КодировкаТекущейВерсии = "";
    Иначе
        КодировкаТекущейВерсии = ПолучитьКодировкуВерсииФайла(ТекущаяВерсияФайла);
	КонецЕсли;    
    //>>
