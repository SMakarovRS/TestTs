...
Функция РазобратьАдресСсылки(Знач АдресСсылки) Экспорт
	...
	ПозицияРазделителя	= СтрНайти(ОстатокСтроки, "/");
	...
	РазделительКоманды	= СтрНайти(ЗначениеСсылки, "?");
...
Функция КонструкторСсылки_getlink(Знач ИмяСтраницы, Знач Параметры = Неопределено) Экспорт
	...
	Возврат АдресСсылкиУниверсальный("getlink", Параметры);
...
//	Строка
//
Функция КонструкторСсылки_openfile(Знач ИмяСтраницы, Знач Параметры = Неопределено) Экспорт
	Если НЕ ТипЗнч(Параметры) = Тип("Структура") Тогда
		Параметры = Новый Структура;
	КонецЕсли;
	...
	Возврат АдресСсылкиУниверсальный("openfile", Параметры);
КонецФункции
