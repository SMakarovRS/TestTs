...
Функция сфпСформироватьСписокОбъектовДляВыбораПоМассивуЗвонящих(МассивЗвонящих, ТолькоПартнеры = Ложь) Экспорт
	...
	Для Каждого Контакт Из МассивЗвонящих Цикл
		Если СтрНайти(Строка(Контакт), НСтр("ru = '<Объект не найден>'")) > 0 Тогда
		...
		Если КонтактИмяМетаданных = "ФизическиеЛица" И НЕ ТолькоПартнеры Тогда
			СписокКонтактов.Добавить(Контакт, Строка(Контакт) + Нстр("ru = ' (физическое лицо)'"));
			
		ИначеЕсли КонтактИмяМетаданных = "Подразделения" И НЕ ТолькоПартнеры Тогда
			СписокКонтактов.Добавить(Контакт, Строка(Контакт) + Нстр("ru = ' (подразделение)'"));
			
		ИначеЕсли КонтактИмяМетаданных = "Пользователи" И НЕ ТолькоПартнеры Тогда
			СписокКонтактов.Добавить(Контакт, Строка(Контакт) + Нстр("ru = ' (пользователь)'"));
			
		ИначеЕсли КонтактИмяМетаданных = "Сотрудники" И НЕ ТолькоПартнеры Тогда
			СписокКонтактов.Добавить(Контакт, Строка(Контакт) + Нстр("ru = ' (сотрудник)'"));
						
		ИначеЕсли КонтактИмяМетаданных = "КонтактныеЛица" Тогда
			СписокКонтактов.Добавить(Контакт, Строка(Контакт) + " <" + Строка(сфпСофтФонПроСервер.сфпПолучитьВладельцаКонтакта(Контакт)) + ">" + Нстр("ru = ' (контактное лицо)'"));
			
		ИначеЕсли КонтактИмяМетаданных = "Контрагенты" Тогда
			СписокКонтактов.Добавить(Контакт, Строка(Контакт) + Нстр("ru = ' (контрагент)'"));
						
		ИначеЕсли КонтактИмяМетаданных = "ПотенциальныеКлиенты" Тогда
...
Процедура сфпСкорректироватьАбонентаУТелефонногоЗвонка(КонтактноеЛицо = Неопределено, Абонент, ТелефонныйЗвонок) Экспорт
	...
	Если КонтактноеЛицо = ТелефонныйЗвонок.АбонентКонтакт Тогда
		Возврат;
	КонецЕсли;
...
Функция сфпНайтиОбъектВРегистреПоТелефону(НомерТелефона) Экспорт
	...
	Если СтрДлина(ОчищенныйНомерТелефона) > 0 Тогда
		...
		Если СтрДлина(ОчищенныйНомерТелефона) > ДлинаВнутреннихНомеров Тогда
			...
			|	Объект КАК Объект,
			|   ВЫБОР КОГДА ТИПЗНАЧЕНИЯ(Объект) = &ТипКонтактныеЛицаПартнеров ТОГДА 1
			|		  КОГДА ТИПЗНАЧЕНИЯ(Объект) = &ТипПартнеры ТОГДА 2
			|         КОГДА ТИПЗНАЧЕНИЯ(Объект) = &ТипКонтрагенты ТОГДА 3
			|         КОГДА ТИПЗНАЧЕНИЯ(Объект) = &ТипКонтактныеЛица ТОГДА 4
			|         КОГДА ТИПЗНАЧЕНИЯ(Объект) = &ТипФизическиеЛица ТОГДА 5
			|         КОГДА ТИПЗНАЧЕНИЯ(Объект) = &ТипПользователи ТОГДА 6
			|         КОГДА ТИПЗНАЧЕНИЯ(Объект) = &ТипСтруктурныеЕдиницы ТОГДА 7
			|         КОГДА ТИПЗНАЧЕНИЯ(Объект) = &ТипОрганизации ТОГДА 8
			|	КОНЕЦ КАК Порядок
			|ИЗ
			...
			Если Метаданные.Справочники.Найти("КонтактныеЛицаПартнеров") <> Неопределено Тогда
				  Запрос.УстановитьПараметр("ТипКонтактныеЛицаПартнеров", Тип("СправочникСсылка.КонтактныеЛицаПартнеров"));
			Иначе Запрос.УстановитьПараметр("ТипКонтактныеЛицаПартнеров", Тип("Число"));
			КонецЕсли;
			Если Метаданные.Справочники.Найти("Партнеры") <> Неопределено Тогда
				  Запрос.УстановитьПараметр("ТипПартнеры", Тип("СправочникСсылка.Партнеры"));
			Иначе Запрос.УстановитьПараметр("ТипПартнеры", Тип("Число"));
			КонецЕсли;
			Если Метаданные.Справочники.Найти("Контрагенты") <> Неопределено Тогда
				  Запрос.УстановитьПараметр("ТипКонтрагенты", Тип("СправочникСсылка.Контрагенты"));
			Иначе Запрос.УстановитьПараметр("ТипКонтрагенты", Тип("Число"));
			КонецЕсли;
			Если Метаданные.Справочники.Найти("КонтактныеЛица") <> Неопределено Тогда
				  Запрос.УстановитьПараметр("ТипКонтактныеЛица", Тип("СправочникСсылка.КонтактныеЛица"));
			Иначе Запрос.УстановитьПараметр("ТипКонтактныеЛица", Тип("Число"));
			КонецЕсли;
			Если Метаданные.Справочники.Найти("ФизическиеЛица") <> Неопределено Тогда
				  Запрос.УстановитьПараметр("ТипФизическиеЛица", Тип("СправочникСсылка.ФизическиеЛица"));
			Иначе Запрос.УстановитьПараметр("ТипФизическиеЛица", Тип("Число"));
			КонецЕсли;
			Если Метаданные.Справочники.Найти("Пользователи") <> Неопределено Тогда
				  Запрос.УстановитьПараметр("ТипПользователи", Тип("СправочникСсылка.Пользователи"));
			Иначе Запрос.УстановитьПараметр("ТипПользователи", Тип("Число"));
			КонецЕсли;
			Если Метаданные.Справочники.Найти("СтруктурныеЕдиницы") <> Неопределено Тогда
				  Запрос.УстановитьПараметр("ТипСтруктурныеЕдиницы", Тип("СправочникСсылка.СтруктурныеЕдиницы"));
			Иначе Запрос.УстановитьПараметр("ТипСтруктурныеЕдиницы", Тип("Число"));
			КонецЕсли;
			Если Метаданные.Справочники.Найти("Организации") <> Неопределено Тогда
				  Запрос.УстановитьПараметр("ТипОрганизации", Тип("СправочникСсылка.Организации"));
			Иначе Запрос.УстановитьПараметр("ТипОрганизации", Тип("Число"));
			КонецЕсли;
		...
		Если ТаблицаРезультата.Количество() > 0 Тогда
			...
			Для Каждого СтрокаРезультата Из ТаблицаРезультата Цикл
				...
				КонтактИмяМетаданных = СтрокаРезультата.Объект.Метаданные().Имя;
				Если КонтактИмяМетаданных = "КонтактныеЛицаПартнеров" Тогда
...
Функция сфпПолучитьВнутреннийНомерПользователяИзКИ(Пользователь = Неопределено) Экспорт
	...
	Если ПустаяСтрока(ИмяПредопределенногоЗначения) Тогда
		Попытка
			ИмяПредопределенногоЗначения = "Справочники.ВидыКонтактнойИнформации.CRM_ТелефонПользователяРабочий";
		Исключение КонецПопытки;
	КонецЕсли;
	Если ПустаяСтрока(ИмяПредопределенногоЗначения) Тогда
		Попытка
			ИмяПредопределенногоЗначения = "Справочники.ВидыКонтактнойИнформации.ТелефонПользователя";
		Исключение КонецПопытки;
	КонецЕсли;
	
	Если НЕ ПустаяСтрока(ИмяПредопределенногоЗначения) Тогда
		Запрос = Новый Запрос("
		...
		|УПОРЯДОЧИТЬ ПО CRM_ОсновнойДляСвязи УБЫВ");
		Запрос.УстановитьПараметр("Пользователь", Пользователь);
...
Функция сфпЗаписатьНомерПользователю(НомерЛинии, Пользователь) Экспорт
	...
	Если НЕ ПустаяСтрока(НомерЛинии) Тогда
		...
		НаборЗаписей.Отбор.Настройка.Установить(ПланыВидовХарактеристик[ИмяПВХ_СофтфонНастройкиПользователей].сфпТекущийВнутреннийНомер);
		НаборЗаписей.Прочитать();
		...
		Если НЕ НомерЗаписан Тогда
			...
			ЗаписьНабора.Настройка = ПланыВидовХарактеристик[ИмяПВХ_СофтфонНастройкиПользователей].сфпТекущийВнутреннийНомер;
			ЗаписьНабора.Значение = НомерЛинии;
		КонецЕсли;	
...
Функция сфпПолучитьМассивВсехПользователей(МассивВидов) Экспорт
	...
	|	ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ Пользователь, Значение
	|	                  ИЗ РегистрСведений." + ИмяРегистраСофтфонНастройкиПользователей + "
	|	                  ГДЕ Настройка = &Настройка) КАК Настройки
...
//
Функция сфпUnJSON(СтрJSON) Экспорт 
	СтрJSON	= сфпЗаменитьНедопустимыеСимволыXML(СтрJSON);	
	Значение = "";
...
//
Функция сфпЗаменитьНедопустимыеСимволыXML(Знач Текст, СимволЗамены = " ") Экспорт
	ПозицияНачала = 1;
	Пока Истина Цикл
...
Процедура сфпПолучитьИсториюЗвонков() Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("сфпИспользоватьСофтФон") <> Истина Тогда
		Возврат;
	КонецЕсли;
...
Процедура сфпЗаписатьИдентификаторЗаписиВРегистр(Звонок, ИдентификаторЗвонка, ИдентификаторЗаписи) Экспорт
	...
	|	НомерТелефона,
	|	Звонок,
	|	Звонок.АбонентКонтакт КАК АбонентКонтакт
	|ИЗ
...
Функция сфпПолучитьКлючСессииCoMagic() Экспорт
	...
	Попытка
		...
		Если Ответ.КодСостояния = 200 Тогда
			...
			РезультатСоответствие = сфпUnJSON(ТекстовыйФайл.ПолучитьТекст());
...
Процедура сфпЗаполнитьБизнесРегионПоНомеру(Объект) Экспорт
	...
	ИначеЕсли Объект.Метаданные().Имя = "ТелефонныйЗвонок" Тогда
		Если ЗначениеЗаполнено(Объект.АбонентКонтакт) Тогда 
			КонтактИмяМетаданных = Объект.АбонентКонтакт.Метаданные().Имя;
			Если КонтактИмяМетаданных = "Партнеры" Тогда
...
Функция сфпИмяФормыДокументаТелефонныйЗвонок() Экспорт
	...
	ИначеЕсли Метаданные.Документы.Найти("CRM_ТелефонныйЗвонок") <> Неопределено Тогда
		ИмяФормаЗвонка = "Документ.CRM_ТелефонныйЗвонок.Форма.ФормаДокумента";
...
Функция сфпИмяФормыСпискаДокументовТелефонныйЗвонок() Экспорт
	...
	Если Метаданные.Документы.Найти("ТелефонныйЗвонок") <> Неопределено Тогда
		ИмяФормаЗвонка = "Документ.ТелефонныйЗвонок.Форма.сфпФормаСписка";
		
	ИначеЕсли Метаданные.Документы.Найти("CRM_ТелефонныйЗвонок") <> Неопределено Тогда
		ИмяФормаЗвонка = "Документ.CRM_ТелефонныйЗвонок.Форма.ФормаСписка";
