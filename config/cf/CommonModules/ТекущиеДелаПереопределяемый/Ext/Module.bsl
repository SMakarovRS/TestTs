...
Процедура ПриОпределенииОбработчиковТекущихДел(Обработчики) Экспорт
	...
	//<< УИТ
	Если ПравоДоступа("Редактирование", Метаданные.Справочники.Дела) Тогда
    	Обработчики.Добавить(Справочники.Дела);
    КонецЕсли;	
	...
	Если ПравоДоступа("Редактирование", Метаданные.Документы.НарядНаРаботы) Тогда
    	Обработчики.Добавить(Документы.НарядНаРаботы);    
    КонецЕсли;
	
	Если ПравоДоступа("Редактирование", Метаданные.Документы.Задание) Тогда
    	Обработчики.Добавить(Документы.Задание);    
    КонецЕсли;	
	
	МодульКонтрольВеденияУчета = ОбщегоНазначения.ОбщийМодуль("КонтрольВеденияУчетаСлужебный");
	ИндексЭлемента = Обработчики.Найти(МодульКонтрольВеденияУчета);
	Если ИндексЭлемента <> Неопределено И ИндексЭлемента > 0 Тогда
		Обработчики.Удалить(ИндексЭлемента);
	КонецЕсли;	
	//>>		
...
Процедура ПриОтключенииТекущихДел(ОтключаемыеДела) Экспорт
	...
	Если Не УправлениеITОтделом8УФПовтИсп.Право("ПолныеПрава") Тогда
		Если Не УправлениеITОтделом8УФПовтИсп.Право("ДобавлениеИзменениеВзаимодействий") Тогда			
			ОтключаемыеДела.Добавить("ВзаимодействияПочта");
		КонецЕсли;
	КонецЕсли;	
	ОтключаемыеДела.Добавить("КлассификаторБанков");
	ОтключаемыеДела.Добавить("КлассификаторВалют");
	//>>	
