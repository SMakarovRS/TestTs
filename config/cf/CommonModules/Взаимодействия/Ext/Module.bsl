...
Функция ВыполнитьОтправкуПисьма(Объект, Соединение = Неопределено, ПараметрыПисьма = Неопределено, ПротоколПочты = "") Экспорт
	...
	//<< УИТ Барилко Виталий - 23.06.2020 11:06:13
...
Процедура ОтразитьРезультатыОтправкиSMSВДокументе(ДокументОбъект, РезультатОтправки)
	...
	Для Каждого ОтправленноеСообщение Из РезультатОтправки.ОтправленныеСообщения Цикл
		//<< УИТ Проставляем идентификаторы.
		...
		//КонецЦикла;
		НомерПолучателя = УправлениеITОтделом8УФ.ФорматироватьНомер(ОтправленноеСообщение.НомерПолучателя);
		Для Каждого Строки Из ДокументОбъект.Адресаты Цикл
			Если УправлениеITОтделом8УФ.ФорматироватьНомер(Строки.НомерДляОтправки) = НомерПолучателя Тогда
				Строки.ИдентификаторСообщения = ОтправленноеСообщение.ИдентификаторСообщения;
				Строки.СостояниеСообщения     = Перечисления.СостоянияСообщенияSMS.ОтправляетсяПровайдером;				
			КонецЕсли;
		КонецЦикла;
		//>>
	КонецЦикла;
