...
// Доступно ли право редактирования статьи базы знаний.
//
// Параметры:
//	ПользовательСсылка - СправочникСсылка.Пользователи - проверяемый пользователь.
//	СтатьяСсылка - СправочникСсылка.СтатьиБазыЗнаний - статья базы знаний.
//
// Возвращаемое значение:
//	Булево - разрешен ли доступ.
//
...
// Доступно ли право просмотра статьи базы знаний.
//
// Параметры:
//	ПользовательСсылка - СправочникСсылка.Пользователи - проверяемый пользователь.
//	СтатьяСсылка - СправочникСсылка.СтатьиБазыЗнаний - статья базы знаний.
...
//
// Параметры:
//	КлючСессии - УникальныйИдентификатор - ключ текущей сессии.
//
...
Процедура ОбновитьРейтингКлючевыхСлов(Знач ПараметрыВыполнения, Знач АдресХранилища = "") Экспорт
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ВложенныйЗапрос.Тег КАК Тег,
		|	ВложенныйЗапрос.КатегорияБазыЗнаний,
		|	КОЛИЧЕСТВО(ВложенныйЗапрос.СтатьяБазыЗнаний) КАК Количество,
		|	ВложенныйЗапрос.Тег.Наименование КАК Наименование
		|ИЗ
		|	(ВЫБРАТЬ
		|		СвязьКлючевыхСловБазыЗнаний.Тег КАК Тег,
		...
		|	ГДЕ
		|		СвязьКлючевыхСловБазыЗнаний.Тег В(&Теги)
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
		...
		|	ГДЕ
		|		СвязьКлючевыхСловБазыЗнаний.Тег В(&Теги)) КАК ВложенныйЗапрос
		|
		|СГРУППИРОВАТЬ ПО
		|	ВложенныйЗапрос.Тег,
		|	ВложенныйЗапрос.КатегорияБазыЗнаний,
		|	ВложенныйЗапрос.Тег.Наименование
		|
		|УПОРЯДОЧИТЬ ПО
		|	Тег";
	
	ТаблицаДанных = Запрос.Выполнить().Выгрузить();
	ТаблицаДанных.Индексы.Добавить("Тег");
	
	// Новые данные рейтинга.
	МассивТегов	= Новый Массив;
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(МассивТегов, ТаблицаДанных.ВыгрузитьКолонку("Тег"), Истина);
	Для Каждого Тег Из МассивТегов Цикл
		ТаблицаЗаписей = ТаблицаДанных.НайтиСтроки(Новый Структура("Тег", Тег));
		
		НаборЗаписей = РегистрыСведений.РейтингКлючевыхСловБазыЗнаний.СоздатьНаборЗаписей();
		НаборЗаписей.Отбор.Тег.Установить(Тег);
		...
		ТекИндекс = КлючевыеСлова.Найти(Тег);
	...
	Для Каждого Тег Из КлючевыеСлова Цикл
		НаборЗаписей = РегистрыСведений.РейтингКлючевыхСловБазыЗнаний.СоздатьНаборЗаписей();
		НаборЗаписей.Отбор.Тег.Установить(Тег);
...
//			Оповещения - Булево.
//
Функция ПолучитьКатегории(Знач НастройкаОтбора, Знач НастройкаПорядка = Неопределено, 
	Знач Разрешенные = Истина) Экспорт
	...
	КомпоновщикНастроек.Настройки.ПараметрыДанных.УстановитьЗначениеПараметра("ТекущийПользователь", ТекПользователь);
...
//		Идентификатор - УникальныйИдентификатор
//
Функция ПолучитьСтатьи(Знач НастройкаОтбора, Знач НастройкаПорядка = Неопределено, Знач Количество = 0, 
	Знач СтраницаНазад = Ложь) Экспорт
...
//		Идентификатор - УникальныйИдентификатор
//
Функция ПолучитьНовости(Знач НастройкаОтбора, Знач НастройкаПорядка = Неопределено, Знач Количество = 20, 
	Знач Разрешенные = Истина) Экспорт
...
//
Функция ПолучитьСтатьиПоСтатистике(Знач ВидСтраницы, Знач СтатьяСсылка = Неопределено, Знач Количество = 20, 
	Знач Направление = "вперед", Знач Разрешенные = Истина) Экспорт
	...
	Если ЗначениеЗаполнено(СтатьяСсылка) Тогда
		...
		Если ВидСтраницы = "featured_articles" Тогда
			// СредняяОценка >	
			ГруппаИЛИ_1 = БазаЗнанийКлиентСервер.ДобавитьГруппуОтбора(НастройкаОтбора, 
			...
			ГруппаИЛИ_2 = БазаЗнанийКлиентСервер.ДобавитьГруппуОтбора(ГруппаИЛИ_1, 
				ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИли);
			БазаЗнанийКлиентСервер.ДобавитьЭлементОтбора(ГруппаИЛИ_2, "СредняяОценка", Статистика.СредняяОценка, 
			...
			ГруппаИЛИ_3 = БазаЗнанийКлиентСервер.ДобавитьГруппуОтбора(ГруппаИЛИ_1, 
				ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИли);
			БазаЗнанийКлиентСервер.ДобавитьЭлементОтбора(ГруппаИЛИ_3, "СредняяОценка", Статистика.СредняяОценка, 
				ВидСравненияКомпоновкиДанных.Равно);
			БазаЗнанийКлиентСервер.ДобавитьЭлементОтбора(ГруппаИЛИ_3, "Просмотры", Статистика.Просмотры, 
				ВидСравненияКомпоновкиДанных.Равно);
		...
		ИначеЕсли ВидСтраницы = "recently_added" Тогда
			...
			ГруппаИЛИ_2 = БазаЗнанийКлиентСервер.ДобавитьГруппуОтбора(ГруппаИЛИ_1, 
				ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИли);
			БазаЗнанийКлиентСервер.ДобавитьЭлементОтбора(ГруппаИЛИ_2, "Создана", Статистика.Создана, 
				ВидСравненияКомпоновкиДанных.Равно);
		...
		ИначеЕсли ВидСтраницы = "most_popular" Тогда
			...
			ГруппаИЛИ_2 = БазаЗнанийКлиентСервер.ДобавитьГруппуОтбора(ГруппаИЛИ_1, 
				ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИли);
			БазаЗнанийКлиентСервер.ДобавитьЭлементОтбора(ГруппаИЛИ_2, "Просмотры", Статистика.Просмотры, 
				ВидСравненияКомпоновкиДанных.Равно);
			БазаЗнанийКлиентСервер.ДобавитьЭлементОтбора(ГруппаИЛИ_2, "Ссылка", СтатьяСсылка, НапрСравнения);
...
//		КонецЕсли;
	...
	НастройкаОтбора = Новый Массив;
	БазаЗнанийКлиентСервер.ДобавитьЭлементОтбора(НастройкаОтбора, "Ссылка", МассивСтатей, 
...
// Возвращает массив ссылок на статьи с отбором по тегу и категории.
//
// Параметры:
//	Тег - СправочникСсылка.Теги.
//	КатегорияСсылка - СправочникСсылка.КатегорииБазыЗнаний.
//
// Возвращаемое значение:
//	Массив[СправочникСсылка.СтатьиБазыЗнаний].
//
Функция ПолучитьСтатьиПоКлючевомуСлову(Знач Тег, Знач КатегорияСсылка) Экспорт
	
	Если ТипЗнч(Тег) <> Тип("СправочникСсылка.Теги") Тогда
	...
	Если ЗначениеЗаполнено(КатегорияСсылка) Тогда
		...
		Запрос.Текст = 
			...
			|		ПО СвязьКлючевыхСловБазыЗнаний.СтатьяБазыЗнаний = СтатьиБазыЗнанийКатегории.Ссылка
			|ГДЕ
			|	СвязьКлючевыхСловБазыЗнаний.Тег = &Тег
...
//		Комментарии - Массив[Структура]. Справочник "Комментарии".
//		КлючевыеСлова - ТаблицаЗначений. Справочник "Теги".
//
// Возвращаемое значение:
//	Структура.
//
Функция ПолучитьДанныеСтатьи(Знач СтатьяСсылка, Знач Область = "") Экспорт
	...
	Если НЕ ЗначениеЗаполнено(Область) Тогда
		Область = "Общее, Содержание, Картинки, Таблицы, Категории, Сноски, ПодсветкаКода,"
				+ "Статистика, Блокировки, КлючевыеСлова, Комментарии, Файлы, ИмпортированныеКартинки";
	КонецЕсли;
	...
	Если НаборДанных.Свойство("Общее") Тогда
		...
		|	СтатьиБазыЗнаний.ОписаниеМетатегов
		|ИЗ
		...
		ДанныеСтатьи.Вставить("Автор"				, Выборка.Автор);
		...
		ДанныеСтатьи.Вставить("РазделИмпортирован"	, Выборка.РазделИмпортирован);
	КонецЕсли;
	...
	Если НаборДанных.Свойство("Содержание") Тогда
		Запрос.Текст = 
			...
			|	СтатьиБазыЗнанийСодержание.РазделИмпортирован,
			|	СтатьиБазыЗнанийСодержание.ОписаниеМетатегов
			|ИЗ
	...
	Если НаборДанных.Свойство("Картинки") Тогда
		Запрос.Текст = 
			...
			|	СтатьиБазыЗнанийКартинки.ОписаниеСтиля
			|ИЗ
		...
		ТаблицаКартинки.Колонки.Добавить("АдресХранилища");
		...
		Для Каждого СтрокаТаблицы Из ТаблицаКартинки Цикл
			...
			Если ЗначениеЗаполнено(СтрокаТаблицы.Файл) Тогда
				Попытка
					ДвоичныеДанные  = РаботаСФайлами.ДвоичныеДанныеФайла(СтрокаТаблицы.Файл);
					СтрокаТаблицы.АдресХранилища = ПоместитьВоВременноеХранилище(ДвоичныеДанные, Новый УникальныйИдентификатор);					
				Исключение
	...
	Если НаборДанных.Свойство("КлючевыеСлова") Тогда
		...
		ЗапросТеги.Текст = 
			"ВЫБРАТЬ
            |   СвязьКлючевыхСловБазыЗнаний.Тег КАК Ссылка,
            |   СвязьКлючевыхСловБазыЗнаний.Тег.Наименование КАК Наименование,
            |   СвязьКлючевыхСловБазыЗнаний.Тег.ЦветФона КАК ЦветФона,
            |   СвязьКлючевыхСловБазыЗнаний.Тег.ЦветТекста КАК ЦветТекста
            |ИЗ
            |   РегистрСведений.СвязьКлючевыхСловБазыЗнаний КАК СвязьКлючевыхСловБазыЗнаний
            |ГДЕ
            |   СвязьКлючевыхСловБазыЗнаний.СтатьяБазыЗнаний = &Ссылка
            |   И НЕ СвязьКлючевыхСловБазыЗнаний.Тег.ПометкаУдаления
            |
            |УПОРЯДОЧИТЬ ПО
            |   Наименование";
	...
	Если НаборДанных.Свойство("Комментарии") Тогда
		...
		ЗапросКомментарии.Текст = 
			...
			|	Комментарии.Автор.Наименование КАК АвторПредставление,
			...
			|		ПО Комментарии.Ссылка = СвязьКомментариев.Ссылка
			|ГДЕ
			|	Комментарии.ВладелецКомментария = &Ссылка
			|	И НЕ Комментарии.ПометкаУдаления";
	...
	Если НаборДанных.Свойство("Файлы") Тогда
		Запрос.Текст = 
			"ВЫБРАТЬ РАЗРЕШЕННЫЕ
			|	СтатьиБазыЗнанийПрисоединенныеФайлы.Ссылка КАК Файл
			|ИЗ
			|	Справочник.СтатьиБазыЗнанийПрисоединенныеФайлы КАК СтатьиБазыЗнанийПрисоединенныеФайлы
			|ГДЕ
			|	СтатьиБазыЗнанийПрисоединенныеФайлы.ВладелецФайла = &Ссылка
			|	И СтатьиБазыЗнанийПрисоединенныеФайлы.Идентификатор = """"";
		
		ТаблицаФайлы = Запрос.Выполнить().Выгрузить();		
		ДанныеСтатьи.Вставить("Файлы", ТаблицаФайлы);
	КонецЕсли;
	
	Если НаборДанных.Свойство("ПодсветкаКода") Тогда
		Запрос.Текст = 
			"ВЫБРАТЬ
		...
		ТаблицаПодсветкаКода = Запрос.Выполнить().Выгрузить();		
		ДанныеТаблицы	= Новый Соответствие;
		Для Каждого СтрокаТаблицы Из ТаблицаПодсветкаКода Цикл
			КлючОбласти		= ?(ПустаяСтрока(СтрокаТаблицы.Область), "Главная", СтрокаТаблицы.Область);
			МассивОбласти	= ДанныеТаблицы.Получить(КлючОбласти);
		...
		ДанныеСтатьи.Вставить("ПодсветкаКода", ДанныеТаблицы);
	КонецЕсли;	
...
Функция ПолучитьМассивКлючевыхСловПоКатегории(Знач КатегорияСсылка, Знач Количество = 0) Экспорт
	...
	Запрос.Текст = 
		...
		|	РейтингКлючевыхСловБазыЗнаний.Тег,
		|	РейтингКлючевыхСловБазыЗнаний.Количество,
		|	РейтингКлючевыхСловБазыЗнаний.Тег.Наименование КАК Наименование
		|ИЗ
...
Процедура ДобавитьНовость(Знач ПараметрыВыполнения, Знач АдресХранилища = "") Экспорт
	...
	ОбъектБазыЗнаний	= ?(ПараметрыВыполнения.Свойство("ОбъектБазыЗнаний"), ПараметрыВыполнения.ОбъектБазыЗнаний, 
		Неопределено);
	...
	Результат = Справочники.НовостиБазыЗнаний.ДобавитьНовость(Автор, Заголовок, ТекстHTML, ОбъектБазыЗнаний, ВидНовости,
		Получатели, Важность, Текст);
...
Процедура УдалениеУстаревшихНовостейБазыЗнаний() Экспорт
		
	ОбщегоНазначения.ПриНачалеВыполненияРегламентногоЗадания(
		Метаданные.РегламентныеЗадания.УдалениеУстаревшихНовостейБазыЗнаний);	
	УдалитьУстаревшиеНовостиБазыЗнаний();
	
КонецПроцедуры

Процедура УдалитьУстаревшиеНовостиБазыЗнаний() Экспорт
	
	УдалитьНовостиБазыЗнанийОбУдаленныхОбъектах();
	
КонецПроцедуры

Процедура УдалитьНовостиБазыЗнанийОбУдаленныхОбъектах() Экспорт
	...
	Запрос.Текст = 
		...
		|	Справочник.НовостиБазыЗнаний КАК НовостиБазыЗнаний
		|ГДЕ
		|	НовостиБазыЗнаний.Вид = &Вид
		|	И НовостиБазыЗнаний.Дата <= &ДатаПроверки";
	
	Запрос.УстановитьПараметр("Вид", Перечисления.ВидыНовостейБазыЗнаний.Удаление);
	Запрос.УстановитьПараметр("ДатаПроверки", ДобавитьМесяц(ТекущаяДатаСеанса(), -1));
	РезультатЗапроса       = Запрос.Выполнить();
	ВыборкаДетальныеЗаписи = РезультатЗапроса.Выбрать();
	
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		НовостьБазыЗнаний = ВыборкаДетальныеЗаписи.Ссылка;
		Попытка						
			ЭлементОбъект = НовостьБазыЗнаний.ПолучитьОбъект();
			ЭлементОбъект.Удалить();
		Исключение
		КонецПопытки;	
	КонецЦикла;
