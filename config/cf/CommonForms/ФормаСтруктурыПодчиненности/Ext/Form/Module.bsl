...
&НаСервере
Перем мДерево, мУжеВСписке; // Системные переменные.

&НаСервере
Перем мКэшРеквизитовДокумента; // Кэш реквизитов.
...
Процедура НайтиВСписке(Команда)
	...
	Попытка
		...
		ФормаСписка.Открыть();
	Исключение
		УправлениеITОтделом8УФ.СообщитьОбОшибке(ДокументСсылка, ОписаниеОшибки());
	КонецПопытки;
