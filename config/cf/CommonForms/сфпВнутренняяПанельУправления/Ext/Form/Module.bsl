...
Процедура ДобавитьОбновитьЗвонок(СтруктураЗвонка)
	...
	Если СтруктураЗвонка.ВходящийЗвонок Тогда
		Если СтруктураЗвонка.ВнешнийЗвонок Тогда
			СтрокаЗвонка.ВидЗвонка	= НСтр("ru = 'Внешний входящий'");
		Иначе
			СтрокаЗвонка.ВидЗвонка	= НСтр("ru = 'Внутренний входящий'");
		КонецЕсли;	
	Иначе
		Если СтруктураЗвонка.ВнешнийЗвонок Тогда
			СтрокаЗвонка.ВидЗвонка	= НСтр("ru = 'Внешний исходящий'");
		Иначе
			СтрокаЗвонка.ВидЗвонка	= НСтр("ru = 'Внутренний исходящий'");
		КонецЕсли;	
	КонецЕсли;
...
Функция ВыбратьКонтактИзАдреснойКниги(ПереводЗвонка, ДополнительныеПараметры)
	...
	КонецЕсли;	
	ОповещениеВыбора = Новый ОписаниеОповещения(ДополнительныеПараметры.ИмяОповещения, ЭтаФорма, ДополнительныеПараметры);
	ОткрытьФорму("ОбщаяФорма.сфпАдреснаяКнига", ПараметрыФормы, ЭтаФорма, , , , ОповещениеВыбора);
