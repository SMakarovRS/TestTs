...
Процедура ЗаписатьАтрибутЭлемента(ЗаписьXML, ИмяАтрибута, ЗначениеАтрибута)
	...
	ЗаписьXML.ЗаписатьАтрибут(ИмяАтрибута, XMLСтрокаСервер(Формат(ЗначениеАтрибута, 
		"ЧРД=.; ЧН=0; ЧГ=0; БЛ=false; БИ=true")));
	
КонецПроцедуры

&НаСервере
Функция XMLСтрокаСервер(Знач Стр)
	Возврат XMLСтрока(Стр);
КонецФункции
...
Процедура ПредложитьПользователюПродолжитьРедактирование(ОписаниеПоля, РедактируемоеПоле)
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("ПредложитьПользователюПродолжитьРедактированиеЗавершение", ЭтотОбъект, 
...
Процедура ПредложитьПользователюПродолжитьРедактированиеЗавершение(Результат, ПередаваемыеПараметры) Экспорт
	...
	Если Результат=КодВозвратаДиалога.Да Тогда
		...
		//@skip-warning
		ОповещениеПриЗавершении = Новый ОписаниеОповещения("НачатьДобавлениеРедактированиеПоля", ЭтотОбъект, 
			ПередаваемыеПараметры);
		ОткрытьФорму("ОбщаяФорма.РедакторЭтикетокФормаРедактированияПоляМакета", 
			Новый Структура("АдресХранилищаСКД", АдресХранилищаСКД),,,,, ОповещениеПриЗавершении);
...
Процедура ОпроситьПользователяОбУдаленииПересекаемыхПолей(ПередаваемыеПараметры)
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("ОпроситьПользователяОбУдаленииПересекаемыхПолейЗавершение", ЭтотОбъект, 
		ПередаваемыеПараметры), ТекстВопроса, РежимДиалогаВопрос.ДаНет);
...
Процедура ОпроситьПользователяОбУдаленииПересекаемыхПолейЗавершение(РезультатВопроса, ПередаваемыеПараметры) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.Нет Тогда
		...
		Для Каждого ТекПоле Из ПересекаемыеПоля Цикл
			...
			ОчиститьОбласть((ТекПоле.Верх-1)/ШагСетки+1, (ТекПоле.Лево-1)/ШагСетки+1, ТекПоле.Низ/ШагСетки, 
				ТекПоле.Право/ШагСетки);
		КонецЦикла;
		...
		Если ПередаваемыеПараметры.Свойство("РедактируемоеПоле") Тогда
			...
			Если ПередаваемыеПараметры.ОчищатьСтароеОформление Тогда
				ОчиститьОбласть((ПередаваемыеПараметры.РедактируемоеПоле.Верх-1)/ШагСетки+1, 
					(ПередаваемыеПараметры.РедактируемоеПоле.Лево-1)/ШагСетки+1, 
					ПередаваемыеПараметры.РедактируемоеПоле.Низ/ШагСетки, 
					ПередаваемыеПараметры.РедактируемоеПоле.Право/ШагСетки);
			КонецЕсли;
...
Процедура НачатьДобавлениеПоля(ВыбранноеПолеСКД=Неопределено)
	...
	ОписаниеПоля.Вставить("Право", ?(Область.Право/(1/ШагСетки)<=ШиринаЭтикетки*(1/ШагСетки), 
		Область.Право/(1/ШагСетки), ШиринаЭтикетки));
	...
	ОповещениеПриЗавершении = Новый ОписаниеОповещения("НачатьДобавлениеРедактированиеПоляЗавершение", ЭтотОбъект, 
		ПередаваемыеПараметры);
	ОткрытьФорму("ОбщаяФорма.РедакторЭтикетокФормаРедактированияПоляМакета", 
		Новый Структура("АдресХранилищаСКД", АдресХранилищаСКД),,,,, ОповещениеПриЗавершении);
...
Процедура ИзменитьГраницыПоля(ИмяПоля, ИндексГраницы)
	...
	ОписаниеПоля.Вставить("Шрифт",					Новый Шрифт(ПолеТабличнойЧасти.ИмяШрифта, 
		?(ЗначениеЗаполнено(ПолеТабличнойЧасти.РазмерШрифта), ПолеТабличнойЧасти.РазмерШрифта, Неопределено), 
		ПолеТабличнойЧасти.Жирный, ПолеТабличнойЧасти.Наклонный, ПолеТабличнойЧасти.Подчеркивание, 
		ПолеТабличнойЧасти.Зачеркивание));
...
Функция РазрешитьИзменениеГраницы(ПолеТабличнойЧасти, ИндексГраницы, ВеличинаИзменения)
	...
	Если ПолеТабличнойЧасти.Лево-ШагСетки + ?(ИндексГраницы = 1, ВеличинаИзменения, 0)>ПолеТабличнойЧасти.Право 
		+ ?(ИндексГраницы = 3, ВеличинаИзменения, 0) 
		ИЛИ ПолеТабличнойЧасти.Верх-ШагСетки 
		+ ?(ИндексГраницы = 2, ВеличинаИзменения, 0)>ПолеТабличнойЧасти.Низ 
		+ ?(ИндексГраницы = 4, ВеличинаИзменения, 0) Тогда
		Возврат Ложь;
	КонецЕсли;
	...
	Для Каждого ТекПоле Из Поля Цикл
		...
		Если ТекПоле <> ПолеТабличнойЧасти Тогда
			...
			Если ((ТекПоле.Лево-ШагСетки<=Лево И Лево<=ТекПоле.Право-ШагСетки) 
				ИЛИ (ТекПоле.Лево-ШагСетки<=Право И Право<=ТекПоле.Право-ШагСетки))
				ИЛИ ((Лево<=ТекПоле.Лево-ШагСетки И ТекПоле.Лево-ШагСетки<=Право) 
				ИЛИ (Лево<=ТекПоле.Право-ШагСетки И ТекПоле.Право-ШагСетки<=Право)) Тогда
				ГоризонтальноеПересечение = Истина;
			КонецЕсли;
			...
			Если ГоризонтальноеПересечение Тогда
				...
				Если ((ТекПоле.Верх-ШагСетки<=Верх И Верх<=ТекПоле.Низ-ШагСетки) 
					ИЛИ (ТекПоле.Верх-ШагСетки<=Низ И Низ<=ТекПоле.Низ-ШагСетки))
					ИЛИ ((Верх<=ТекПоле.Верх-ШагСетки И ТекПоле.Верх-ШагСетки<=Низ) 
					ИЛИ (Верх<=ТекПоле.Низ-ШагСетки И ТекПоле.Низ<=Низ-ШагСетки)) Тогда
					ВертикальноеПересечение = Истина;
				КонецЕсли;
...
Процедура ИзменитьКоординатуГраницу(ИмяПоля, ИндексГраницы, ВеличинаИзменения)
	...
	Если РазрешитьИзменениеГраницы(ПолеТабличнойЧасти, ИндексГраницы, ВеличинаИзменения) Тогда
		...
		ОчиститьОбласть((ПолеТабличнойЧасти.Верх-1)/ШагСетки+1, (ПолеТабличнойЧасти.Лево-1)/ШагСетки+1, 
			ПолеТабличнойЧасти.Низ/ШагСетки, ПолеТабличнойЧасти.Право/ШагСетки);
		...
		ОписаниеПоля.Вставить("РамкаСправа",			ПолеТабличнойЧасти.РамкаСправа);
		ОписаниеПоля.Вставить("РамкаСнизу",				ПолеТабличнойЧасти.РамкаСнизу);
		ОписаниеПоля.Вставить("ТипРамки",				?(ЗначениеЗаполнено(ПолеТабличнойЧасти.ТипРамки), 
			ПолеТабличнойЧасти.ТипРамки, "Solid"));
		ОписаниеПоля.Вставить("ТолщинаРамки",			?(ЗначениеЗаполнено(ПолеТабличнойЧасти.ТолщинаРамки), 
			ПолеТабличнойЧасти.ТолщинаРамки, 1));
		ОписаниеПоля.Вставить("ТипШтрихкода",			ПолеТабличнойЧасти.ТипШтрихкода);
		ОписаниеПоля.Вставить("РазмерШрифтаПодписи",	ПолеТабличнойЧасти.РазмерШрифтаПодписи);
		ОписаниеПоля.Вставить("ПодписьШтрихкода",		ПолеТабличнойЧасти.ПодписьШтрихкода);
...
Процедура УстановитьВертикальноеПоложениеТекста(ТД, Значение)
	...
	ОписаниеПоля.Вставить("Шрифт",					Новый Шрифт(ТД.ИмяШрифта, ?(ЗначениеЗаполнено(ТД.РазмерШрифта), 
		ТД.РазмерШрифта, Неопределено), ТД.Жирный, ТД.Наклонный, ТД.Подчеркивание, ТД.Зачеркивание));
