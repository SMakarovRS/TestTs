...
Процедура ЗаполнитьТипыОбъектовВДеревеЗначений()
	...
	Для Каждого Справочник Из МетаданныеСправочники Цикл
		...
		Для Каждого ТипВладельца Из ТипыВладельцевФайлов Цикл
			...
			//<< УИТ Устраняем ошибку открытия очистки файлов.				
			...
			//КонецЕсли;
			МетаданныеВладельцаИерархический = Ложь;
			Попытка
				МетаданныеВладельцаИерархический = МетаданныеВладельца.Иерархический;
			Исключение
			КонецПопытки;
			Если МетаданныеСправочники.Найти(МетаданныеВладельца.Имя) <> Неопределено И МетаданныеВладельцаИерархический Тогда
				НоваяСтрока.ВозможностьДетализации = Истина;
			КонецЕсли;
			//>>		
