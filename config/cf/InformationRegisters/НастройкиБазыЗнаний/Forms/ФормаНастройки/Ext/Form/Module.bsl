...
Процедура ЛокальныйКаталогПриИзменении(Элемент)
	...
	Если НЕ Лев(ЛокальныйКаталог, 2) = "\\" Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю("В качестве локального каталога может быть использован только сетевой каталог");
		ЛокальныйКаталог = "";
	КонецЕсли;
КонецПроцедуры
...
Процедура ПрочитатьНаСервере()
	...
	Для Каждого ЗаписьРегистра Из НаборЗаписей Цикл
		...
		Если НЕ ОбщиеНастройки.Свойство(ИмяНастройки) Тогда
			ОбщегоНазначения.СообщитьПользователю("Не найдены параметры настройки """ + СокрЛП(ИмяНастройки) + """ !!!");
			Продолжить;
		КонецЕсли;
