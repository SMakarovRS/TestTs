...
Процедура ЗарегистрироватьШтрихкодыНаСервере()
	...
	Для каждого СтрокаШтрихкода Из ШтрихкодыНоменклатуры Цикл
		...
		Попытка
			...
			Если ЗначениеЗаполнено(СтрокаШтрихкода.Номенклатура) Тогда
				Если ТипЗнч(СтрокаШтрихкода.Номенклатура) = Тип("СправочникСсылка.Номенклатура") Тогда
					МенеджерЗаписи.ЕдиницаИзмерения = СтрокаШтрихкода.Номенклатура.ЕдиницаИзмерения;
				Иначе
					МенеджерЗаписи.ЕдиницаИзмерения = СтрокаШтрихкода.Номенклатура.Владелец.ЕдиницаИзмерения;
				КонецЕсли;
			КонецЕсли;
...
Функция ПолучитьДанныеШтрихкода(Штрихкод)
	...
	Если Выборка.Следующий() Тогда
		ДанныеШтрихкода = Новый Структура("Номенклатура");
...
Процедура Отмена(Команда, Отказ = Ложь)
	...
	Если НЕ ОбработкаЗакрытия Тогда
		ОписаниеОповещения = Новый ОписаниеОповещения("ОтменаЗавершение",ЭтаФорма);
